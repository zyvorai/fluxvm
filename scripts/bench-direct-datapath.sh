...
#
# Knobs: BENCH_RUNS (3) BENCH_DURATION seconds per iperf3 run (5) BENCH_PING_COUNT
...
declare -A HOPS=([floor]=2 [cni-bridge]=8 [standalone-bridge]=5)
...
measure() {
  local topo="$1" target="$2" i out p50 p99 loss val extra tcpv
...
main() {
  ...
  for t in $TOPOLOGIES; do
    case "$t" in
      ...
      *) say "unknown topology '${t}' (floor|cni-bridge|standalone-bridge)" >&2; continue ;;
    esac
    measure "$t" "$GUEST_IP" || true
    teardown_topology
  done
}
