...
if [[ -f /usr/include/bpf/libbpf.h ]] && command -v pkg-config >/dev/null && pkg-config --exists libbpf; then cc -fsyntax-only -Wall -Wextra -Werror $(pkg-config --cflags libbpf) "$ROOT/tools/fluxvm-intelligence-loader.c"; else echo 'SKIP loader compile: libbpf development headers not installed'; fi
