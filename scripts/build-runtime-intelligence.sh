...
for bin in fluxvm-intelligence fluxvm-guard fluxvm-qos fluxvm-shield fluxvm-tcpintel fluxvm-netintel; do cp "$ROOT/target/release/$bin" "$OUT/bin/$bin"; done
echo "runtime intelligence + guard + Set 6 network intelligence artifacts are in $OUT/bin and $OUT/bpf"
