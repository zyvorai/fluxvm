...
for bin in fluxvm-intelligence fluxvm-guard fluxvm-qos; do cp "$ROOT/target/release/$bin" "$OUT/bin/$bin"; done
echo "runtime intelligence + guard artifacts are in $OUT/bin and $OUT/bpf"
