...
for bin in fluxvm-intelligence fluxvm-guard fluxvm-qos fluxvm-shield fluxvm-tcpintel fluxvm-netintel fluxvm-memprof fluxvm-topology; do cp "$ROOT/target/release/$bin" "$OUT/bin/$bin"; done
echo "runtime intelligence + guard + Set 6 network intelligence + Set 7 memory profiler + Set 8 topology intelligence artifacts are in $OUT/bin and $OUT/bpf"
