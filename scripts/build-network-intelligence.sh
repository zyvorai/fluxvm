...
for src in fluxvm_xdp_shield fluxvm_tcp_intel; do
  clang -O2 -g -target bpf -D"__TARGET_ARCH_${TARGET_ARCH}" "${INC[@]}" \
    -I/usr/include -I"$ROOT/bpf" -Wall -Werror \
    -c "$ROOT/bpf/${src}.bpf.c" -o "$OUT/${src}.bpf.o"
done
echo "Set 6 BPF objects: $OUT/fluxvm_xdp_shield.bpf.o $OUT/fluxvm_tcp_intel.bpf.o"
