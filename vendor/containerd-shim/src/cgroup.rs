...
pub fn set_cgroup_and_oom_score(pid: u32) -> Result<()> {
    ...
    // containerd ≥ 2.3 always writes BootstrapParams on stdin. The 0.11
    // crate still expects legacy Any(Options). BootstrapParams often parses
    // as a plausible Any whose value is the namespace (e.g. "k8s.io"), then
    // Options::parse_from_bytes fails with IncorrectTag(46) (ASCII '.').
    // Optional shim_cgroup placement must never abort shim start.
    if !data.is_empty() {
        match Any::parse_from_bytes(&data).and_then(|any| Options::parse_from_bytes(&any.value)) {
            Ok(opts) if !opts.shim_cgroup.is_empty() => {
                add_task_to_cgroup(opts.shim_cgroup.as_str(), pid)?;
            }
            Ok(_) => {}
            Err(err) => {
                log::warn!(
                    "skipping shim_cgroup placement; stdin is not legacy Any(Options) \
