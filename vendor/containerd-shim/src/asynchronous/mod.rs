...
pub fn run_info() -> Result<RuntimeInfo> {
    ...
    // get BinaryName from stdin (legacy Any(Options); containerd ≥ 2.3 may
    // send BootstrapParams — ignore and keep the default binary name).
    if !data.is_empty() {
        match Any::parse_from_bytes(&data).and_then(|any| Options::parse_from_bytes(&any.value)) {
            Ok(opts) if !opts.binary_name().is_empty() => {
                binary_name = opts.binary_name().to_string();
            }
            Ok(_) => {}
            Err(err) => {
                warn!(
                    "stdin is not legacy Any(Options); using default binary name \
                     (containerd ≥ 2.3 BootstrapParams is expected): {err}"
                );
            }
        }
    }
    ...
    T: Shim + Send + Sync + 'static,
{
    ...
    // -info must not require TTRPC_ADDRESS / a running task socket.
    if flags.info {
        let info = match run_info() {
            Ok(info) => info,
            Err(err) => {
                warn!("run_info failed ({err}); returning minimal RuntimeInfo");
                introspection::RuntimeInfo {
        ...
        stdout
            .write_all(&bytes)
            .await
            .map_err(io_error!(e, "write info"))?;
        stdout.flush().await.map_err(io_error!(e, "flush info"))?;
        return Ok(());
    }
    if flags.version {
        println!("{} {}", runtime_id, env!("CARGO_PKG_VERSION"));
        return Ok(());
    }

    // containerd ≥ 2.3 BootstrapParams may omit TTRPC_ADDRESS while still
    // passing -address. Derive the conventional sibling ttrpc socket.
    let ttrpc_address = match env::var(TTRPC_ADDRESS) {
        Ok(v) => v,
        Err(_) if !flags.address.is_empty() => {
            let derived = format!("{}.ttrpc", flags.address);
            warn!(
                "{TTRPC_ADDRESS} unset; deriving {derived} from -address (containerd ≥ 2.3)"
            );
            derived
        }
