...
pub struct CreateVmRequest {
    ...
    /// normal boot and opens `disk` with `locking=off`.
    ///
    /// Safety: `locking=off` is only ever needed on the RECEIVING side of a
    /// migration. QEMU's image locking is a per-opener, cooperative check
    /// (fcntl OFD locks) -- a process that opens with `locking=off` never
    /// takes or checks that lock, so it is never blocked by another still
    /// running QEMU process's (the source's) default `locking=on` lock. The
    /// source therefore needs no change at all; this field is intentionally
    /// never exposed for a normal `create()`/`start()` request. This is
    /// safe ONLY because a receiver process makes no I/O of its own against
    /// `disk` until QEMU's own incoming-migration stream actually
    /// completes -- it is not a general statement that two QEMU processes
    /// may safely share a qcow2 file.
    #[serde(default)]
