        ...
        unsafe {
            match self {
                ...
                ProcIo::Pipes {
                    ...
                    for fd in [stdin_write.take(), stdout_read.take(), stderr_read.take()]
                        .into_iter()
                        .flatten()
                    {
                        libc::close(fd);
...
impl ProcHandle {
            ...
            ProcIo::Pipes {
                ...
                IoStreamKind::Stdin => (
                    stdin_write
                        .as_ref()
                        .copied()
                        .context("stdin stream is absent")?,
                    stdin_attached,
                    "stdin pipe",
                ),
                IoStreamKind::Stdout => (
                    stdout_read
                        .as_ref()
                        .copied()
                        .context("stdout stream is absent")?,
                    stdout_attached,
                    "stdout pipe",
                ),
                IoStreamKind::Stderr => (
                    stderr_read
                        .as_ref()
                        .copied()
                        .context("stderr stream is absent")?,
                    stderr_attached,
                    "stderr pipe",
                ),
            },
    ...
    fn resize_pty(&self, width: u32, height: u32) -> Result<()> {
        ...
        let ProcIo::Pty { master_fd, .. } = &*io else {
            bail!("process is not a TTY");
        };
        let ws = libc::winsize {
    ...
    fn close_stdin(&self) -> Result<()> {
        ...
        match &mut *io {
            ...
            ProcIo::Pipes { stdin_write, .. } => {
                if let Some(fd) = stdin_write.take() {
                    unsafe { libc::close(fd) };
                }
                Ok(())
            }
            ProcIo::Pty { master_fd, .. } => {
                ...
                if rc < 0 {
                    bail!("writing PTY EOF: {}", std::io::Error::last_os_error());
                }
                Ok(())
            }
        }
...
fn handle_stream_connection(mut socket: File, expected_token: Option<&str>) -> Result<()> {
    let mut line = String::new();
    {
        ...
        let n = reader
            .read_line(&mut line)
            .context("reading stdio attach request")?;
        if n == 0 {
            return Ok(());
        }
    }
    if line.len() > 64 * 1024 {
        socket.write_all(
            encode_line(&IoStreamAck {
                ok: false,
                message: Some("attach request too large".into()),
            })?
            .as_bytes(),
        )?;
        socket.flush()?;
    ...
    let result: Result<()> = (|| {
        match stream_kind {
            IoStreamKind::Stdin => {
                ...
                std::io::copy(&mut socket, &mut endpoint).context("streaming container stdin")?;
                endpoint.flush().ok();
            }
...
fn dispatch(request: ContainerRequest) -> Result<ContainerResponse> {
    match request {
        ...
        ContainerRequest::Pids { id } => Ok(ContainerResponse::Pids {
            pids: container_pids(&id)?,
        }),
        ContainerRequest::Stats { id } => Ok(ContainerResponse::Stats {
            stats: container_stats(&id)?,
        }),
        ContainerRequest::CgroupEvents { id } => Ok(ContainerResponse::CgroupEvents {
            events: container_cgroup_events(&id)?,
        }),
        ContainerRequest::SecurityStats => Ok(ContainerResponse::SecurityStats {
            stats: security_stats_snapshot(),
        }),
        ...
        ContainerRequest::Delete { id, exec_id, force } => {
            delete_process(&id, exec_id.as_deref(), force)
        }
    }
}
...
fn create_container(
    ...
    match attach_guest_network_policy(&cgroup_path) {
        Ok(()) => {
            if let Err(e) =
                configure_container_policy(cgroup_id_for(&cgroup_path).unwrap_or(0), network_policy)
            {
    ...
    let ns_request = NsRequest::Create {
        is_sandbox,
        share_process_namespace,
    };
...
fn delete_process(id: &str, exec_id: Option<&str>, force: bool) -> Result<ContainerResponse> {
    ...
    Ok(ContainerResponse::Deleted {
        ...
        exited_at_unix_nano: final_state
            .exited_at_unix_nano
        ...
        if let Err(first) = mount_result {
            if fs_type == "cgroup" {
                mount_one("cgroup2", &target, Some("cgroup2"), flags, data.as_deref())
                    .with_context(|| {
                        format!(
                            "mounting cgroup/cgroup2 on {} after {first:#}",
                            target.display()
                        )
                    })?;
            } else {
                return Err(first)
                    .with_context(|| format!("mounting {source:?} on {}", target.display()));
            }
        }
        ...
        if let Ok(entries) = std::fs::read_dir("/sys/class/block") {
            for entry in entries.flatten() {
                ...
                for serial_path in [sys.join("device/serial"), sys.join("serial")] {
                    let Ok(found) = std::fs::read_to_string(&serial_path) else {
                        continue;
                    };
...
fn create_oci_devices(config: &Value, rootfs: &PathBuf) -> Result<Vec<PathBuf>> {
    let mut mounted = Vec::new();
    let Some(devices) = config.pointer("/linux/devices").and_then(Value::as_array) else {
        return Ok(mounted);
    };
    for dev in devices {
        let path = dev
            .get("path")
            .and_then(Value::as_str)
            .context("OCI device.path is required")?;
        ...
        let kind = dev
            .get("type")
            .and_then(Value::as_str)
            .context("OCI device.type is required")?;
        let file_mode =
            dev.get("fileMode").and_then(Value::as_u64).unwrap_or(0o666) as libc::mode_t;
        ...
        let mode = match kind {
            "c" | "u" => {
                ...
                let meta = std::fs::metadata(&source).with_context(|| {
                    format!("OCI character device {path} is not present in the guest")
                })?;
        ...
        if rc != 0 {
            return Err(std::io::Error::last_os_error())
                .with_context(|| format!("creating OCI device {}", target.display()));
        }
...
fn apply_oci_sysctls(config: &Value) -> Result<()> {
    let Some(sysctls) = config.pointer("/linux/sysctl").and_then(Value::as_object) else {
        return Ok(());
    };
    for (key, value) in sysctls {
        let value = value
            .as_str()
            .context("OCI sysctl value must be a string")?;
        if key.contains('/') || key.contains("..") {
            bail!("unsafe OCI sysctl key {key:?}");
        }
        let path = PathBuf::from("/proc/sys").join(key.replace('.', "/"));
        std::fs::write(&path, value)
            .with_context(|| format!("applying OCI sysctl {key}={value}"))?;
    }
    Ok(())
}
...
fn seccomp_action(raw: &str, errno_ret: Option<u32>) -> Result<u32> {
    ...
    Ok(match raw {
        ...
        "SCMP_ACT_ERRNO" => {
            ...
            if errno > 0xffff {
                bail!("OCI seccomp errnoRet {errno} exceeds the kernel 16-bit action data field");
            }
...
fn parse_seccomp_notify_policy(config: &Value) -> Result<SeccompNotifyPolicy> {
    ...
    let errno = annotations
        ...
        .map(|v| {
            v.parse::<i32>()
                .context("parsing io.zyvor.seccomp.notify.errno")
        })
        .transpose()?
...
fn seccomp_profile_uses_notify(profile: &SeccompProfile) -> bool {
    seccomp_action_base(profile.default_action) == SCMP_ACT_NOTIFY
        || profile
            .rules
            .iter()
            .any(|rule| seccomp_action_base(rule.action) == SCMP_ACT_NOTIFY)
}
...
fn parse_seccomp_profile(config: &Value) -> Result<Option<SeccompProfile>> {
    ...
    let default_action = seccomp_action(
        seccomp
            .get("defaultAction")
            .and_then(Value::as_str)
            .context("OCI seccomp.defaultAction is required")?,
        seccomp
            .get("defaultErrnoRet")
    ...
    if seccomp_action_base(default_action) == SCMP_ACT_NOTIFY {
        bail!(
            "SCMP_ACT_NOTIFY is not supported as seccomp.defaultAction because the listener bootstrap syscalls would deadlock; use explicit NOTIFY syscall rules"
        );
    }
    ...
    if let Some(items) = seccomp.get("syscalls").and_then(Value::as_array) {
        for item in items {
            let action = seccomp_action(
                item.get("action")
                    .and_then(Value::as_str)
                    .context("OCI seccomp syscall action is required")?,
                item.get("errnoRet")
                    .and_then(Value::as_u64)
                    .map(|v| v as u32),
            )?;
            let names = item
                .get("names")
                .and_then(Value::as_array)
                .context("OCI seccomp syscall names are required")?
                .iter()
                .map(|v| {
                    v.as_str()
                        .map(str::to_string)
                        .context("seccomp syscall name must be a string")
                })
                .collect::<Result<Vec<_>>>()?;
            if names.is_empty() {
                bail!("OCI seccomp syscall rule must contain at least one name");
            }
            if seccomp_action_base(action) == SCMP_ACT_NOTIFY
                && names.iter().any(|name| name == "sendmsg")
            {
                bail!(
                    "seccomp NOTIFY on sendmsg is unsafe for FluxVM because sendmsg transfers the notification listener to the guest supervisor"
                );
            }
            ...
            if let Some(items) = item.get("args").and_then(Value::as_array) {
                for arg in items {
                    let index = arg
                        .get("index")
                        .and_then(Value::as_u64)
                        .context("seccomp argument index is required")?;
                    if index > 5 {
                        bail!(
                            "seccomp argument index {index} is outside the Linux syscall ABI range 0..=5"
                        );
                    }
                    let op_raw = arg
                        .get("op")
                        .and_then(Value::as_str)
                        .context("seccomp argument op is required")?;
...
fn unix_fd_socketpair() -> Result<[RawFd; 2]> {
    ...
    if rc != 0 {
        return Err(std::io::Error::last_os_error())
            .context("creating seccomp notification socketpair");
    }
    Ok(fds)
}
...
fn send_fd(socket: RawFd, fd: RawFd) -> Result<()> {
    ...
    if cmsg.is_null() {
        bail!("building SCM_RIGHTS header failed");
    }
    unsafe {
...
fn recv_fd(socket: RawFd) -> Result<RawFd> {
    ...
    if rc <= 0 {
        return Err(if rc == 0 {
            std::io::Error::new(
                ...
                "seccomp notification socket closed before fd transfer",
            )
        } else {
            std::io::Error::last_os_error()
        })
    ...
    if fd < 0 {
        bail!("received invalid seccomp notification fd");
    }
    Ok(fd)
}
...
fn wait_for_seccomp_listener(socket: RawFd, target_pid: u32) -> Result<RawFd> {
    loop {
        ...
        if rc < 0 {
            ...
            return Err(std::io::Error::last_os_error())
                .context("polling seccomp listener bootstrap socket");
        }
        if rc == 0 {
            if !process_still_exists(target_pid) {
                bail!(
                    "container process exited before transferring the seccomp notification listener"
                );
            }
            continue;
        }
...
fn run_seccomp_notify_broker(
    ...
    let notify_fd = match received {
        ...
        Err(e) => {
            security_counters()
                ...
                .fetch_add(1, Ordering::Relaxed);
            eprintln!(
                "security audit: seccomp-notify broker {label} failed to receive listener: {e:#}"
            );
            return;
        }
    };
    ...
    type NotifyAlloc =
        unsafe extern "C" fn(*mut *mut ScmpNotif, *mut *mut ScmpNotifResp) -> libc::c_int;
    ...
    let soname = match CString::new("libseccomp.so.2") {
        Ok(v) => v,
        Err(_) => return,
    };
    ...
    let run = (|| -> Result<()> {
        ...
        let receive: NotifyReceive =
            unsafe { dlsym_required(handle, b"seccomp_notify_receive\0")? };
        let respond: NotifyRespond =
            unsafe { dlsym_required(handle, b"seccomp_notify_respond\0")? };
        let id_valid: NotifyIdValid =
            unsafe { dlsym_required(handle, b"seccomp_notify_id_valid\0")? };
        ...
        loop {
            ...
            if poll_rc < 0 {
                ...
                bail!(
                    "polling seccomp notification fd: {}",
                    std::io::Error::last_os_error()
                );
            }
            if poll_rc == 0 {
            ...
            if rc != 0 {
                if rc == -libc::EINTR || rc == -libc::ENOENT {
                    continue;
                }
                bail!("seccomp_notify_receive failed: {rc}");
            }
            security_counters()
                .seccomp_notify_received
                .fetch_add(1, Ordering::Relaxed);
            let request = unsafe { *buffers.req };
            if unsafe { id_valid(notify_fd, request.id) } != 0 {
                continue;
            }
            ...
            *response = ScmpNotifResp::default();
            ...
            match policy.mode {
                SeccompNotifyMode::Deny => {
                    ...
                    security_counters()
                        ...
                        .fetch_add(1, Ordering::Relaxed);
                    eprintln!(
                        "security audit: seccomp-notify {label} nr={} pid={} decision=deny errno={}",
                        request.data.nr, request.pid, policy.errno
                    );
                }
            ...
            if rc != 0 && rc != -libc::ENOENT {
                bail!("seccomp_notify_respond failed: {rc}");
            }
        }
        Ok(())
    })();
...
unsafe fn dlsym_required<T: Copy>(handle: *mut libc::c_void, name: &[u8]) -> Result<T> {
    ...
    if ptr.is_null() {
        bail!(
            "dynamic security library symbol {} is missing",
...
fn apply_seccomp_on_handle(
    ...
    type RuleAddArray = unsafe extern "C" fn(
        *mut libc::c_void,
        u32,
        libc::c_int,
        u32,
...
fn validate_selinux_mount_label(label: &str) -> Result<()> {
    ...
    if label.contains('"') || label.contains('\\') {
        bail!(
            "invalid OCI linux.mountLabel: quote/backslash characters are not supported in mount options"
        );
    }
    ...
    if handle.is_null() {
        security_counters()
            ...
            .fetch_add(1, Ordering::Relaxed);
        bail!(
            "OCI SELinux mount label requested but libselinux.so.1 is not installed in the guest"
        );
    }
    let result = (|| -> Result<()> {
        ...
        if unsafe { is_enabled() } <= 0 {
            bail!(
                "OCI SELinux mount label {label:?} requested but SELinux is not enabled in the guest"
            );
        }
        Ok(())
    })();
...
fn format_selinux_mount_data(
    ...
    let Some(label) = mount_label else {
        return Ok(data.map(str::to_string));
    };
    if data.is_some_and(|d| {
        d.split(',').any(|part| {
            part.starts_with("context=")
                || part.starts_with("fscontext=")
                || part.starts_with("defcontext=")
                || part.starts_with("rootcontext=")
        })
    }) {
        bail!(
            "OCI mount data already contains an SELinux context option while linux.mountLabel is set"
        );
    }
...
fn validate_lsm_string(kind: &str, value: &str) -> Result<()> {
    ...
    if value.len() > 4096 {
        bail!("invalid OCI {kind}: value is too long");
    }
    Ok(())
}
...
fn apply_apparmor_on_exec(profile: &str) -> Result<()> {
    ...
    if !enabled {
        security_counters()
            ...
            .fetch_add(1, Ordering::Relaxed);
        bail!(
            "OCI AppArmor profile {profile:?} requested but AppArmor is not enabled in the guest"
        );
    }
    let candidates = [
    ...
    for path in candidates {
        match OpenOptions::new().write(true).open(path) {
            ...
            Err(e) => {
                return Err(e).with_context(|| format!("opening AppArmor exec attribute {path}"));
            }
        }
    }
    Err(last.unwrap_or_else(|| {
        std::io::Error::new(
            ...
            "AppArmor exec attribute missing",
        )
    }))
...
fn apply_selinux_on_exec(label: &str) -> Result<()> {
    ...
    let handle = unsafe { libc::dlopen(soname.as_ptr(), libc::RTLD_NOW | libc::RTLD_LOCAL) };
    if handle.is_null() {
        bail!("OCI SELinux label requested but libselinux.so.1 is not installed in the guest");
    }
    let result = (|| -> Result<()> {
        let is_enabled: IsEnabled = unsafe { dlsym_required(handle, b"is_selinux_enabled\0")? };
        let setexeccon: SetExecCon = unsafe { dlsym_required(handle, b"setexeccon\0")? };
        if unsafe { is_enabled() } <= 0 {
            bail!("OCI SELinux label {label:?} requested but SELinux is not enabled in the guest");
        }
        let label = CString::new(label.as_bytes())?;
        if unsafe { setexeccon(label.as_ptr()) } != 0 {
            return Err(std::io::Error::last_os_error())
                .context("applying OCI SELinux process label");
        }
        Ok(())
    })();
    unsafe {
...
fn parse_device_access(value: &str) -> Result<u32> {
    ...
    if mask == 0 {
        bail!("OCI device cgroup rule access must not be empty");
    }
    Ok(mask)
}
...
fn parse_device_cgroup_policy(config: &Value) -> Result<Option<DeviceCgroupPolicy>> {
    let Some(items) = config
        .pointer("/linux/resources/devices")
        .and_then(Value::as_array)
    else {
        return Ok(None);
    };
    ...
    if items.len() > 128 {
        bail!("OCI device cgroup policy has too many rules (max 128)");
    }
    ...
    for item in items {
        let allow = item
            .get("allow")
            .and_then(Value::as_bool)
            .context("OCI device cgroup rule allow is required")?;
        ...
        let access =
            parse_device_access(item.get("access").and_then(Value::as_str).unwrap_or("rwm"))?;

        if dev_type.is_none() {
            if major.is_some() || minor.is_some() || access != 0x7 {
                bail!(
                    "OCI wildcard device rule must be type='a', major/minor wildcards, access='rwm'"
                );
            }
            default_allow = allow;
            rules.clear();
        ...
        let same_meta =
            |r: &DeviceCgroupRule| r.dev_type == dev_type && r.major == major && r.minor == minor;
        if allow != default_allow {
            if let Some(existing) = rules.iter_mut().find(|r| same_meta(r)) {
                existing.access |= access;
            } else {
                rules.push(DeviceCgroupRule {
                    allow,
                    dev_type,
                    major,
                    minor,
        ...
        if punches_wildcard {
            bail!(
                "OCI device cgroup rule would punch a partial hole in an existing wildcard exception"
            );
        }
...
fn bpf_mov64_imm(dst: u8, imm: i32) -> BpfInsn {
    BpfInsn::new(0xb7, dst, 0, 0, imm)
}
fn bpf_mov64_reg(dst: u8, src: u8) -> BpfInsn {
    BpfInsn::new(0xbf, dst, src, 0, 0)
}
fn bpf_ldxw(dst: u8, src: u8, off: i16) -> BpfInsn {
    BpfInsn::new(0x61, dst, src, off, 0)
}
fn bpf_and64_imm(dst: u8, imm: i32) -> BpfInsn {
    BpfInsn::new(0x57, dst, 0, 0, imm)
}
fn bpf_rsh64_imm(dst: u8, imm: i32) -> BpfInsn {
    BpfInsn::new(0x77, dst, 0, 0, imm)
}
fn bpf_jne_imm(dst: u8, imm: i32, off: i16) -> BpfInsn {
...
fn build_device_bpf(policy: &DeviceCgroupPolicy) -> Result<Vec<BpfInsn>> {
    ...
    for rule in &policy.rules {
        let dev_type = rule
            .dev_type
            .context("normalized device BPF rule is missing a type")?;
        ...
        if let Some(major) = rule.major {
            if major > i32::MAX as u32 {
                bail!("device cgroup major {major} exceeds eBPF immediate range");
            }
            jumps.push(out.len());
            out.push(bpf_jne_imm(4, major as i32, 0));
        }
        if let Some(minor) = rule.minor {
            if minor > i32::MAX as u32 {
                bail!("device cgroup minor {minor} exceeds eBPF immediate range");
            }
            jumps.push(out.len());
        ...
        for index in jumps {
            let skip = end
                .checked_sub(index + 1)
                .context("device BPF jump underflow")?;
            if skip > i16::MAX as usize {
                bail!("device cgroup BPF rule is too large");
            }
    ...
    out.push(bpf_exit());
    if out.len() > 4096 {
        bail!("device cgroup BPF program exceeds the conservative 4096 instruction limit");
    }
    Ok(out)
}
...
fn attach_device_cgroup_policy(path: &std::path::Path, policy: &DeviceCgroupPolicy) -> Result<()> {
    ...
    if prog_fd < 0 {
        ...
        if verifier.is_empty() {
            return Err(err).context("loading cgroup-v2 device BPF program");
        }
    ...
    let cgroup_fd = File::open(path)
        .with_context(|| {
            format!(
                "opening container cgroup {} for device BPF attach",
                path.display()
            )
        })?
        .into_raw_fd();
    ...
    if let Some(err) = attach_error {
        return Err(err).with_context(|| {
            format!(
                "attaching cgroup-v2 device BPF program to {}",
                path.display()
            )
        });
    }
...
fn safe_guest_destination(destination: &str) -> Result<PathBuf> {
    ...
    if !path.is_absolute() {
        bail!("OCI mount destination must be absolute: {destination}");
    }
    ...
    for opt in options {
        match opt.as_str() {
            ...
            other
                if !matches!(
                    other,
                    "remount" | "private" | "rprivate" | "slave" | "rslave" | "shared" | "rshared"
                ) =>
            {
...
fn mount_one(
    ...
    if rc != 0 {
        bail!("mount: {}", std::io::Error::last_os_error());
    }
    Ok(())
}
...
fn resource_limits_from_oci(config: &Value) -> ResourceLimits {
    ...
    ResourceLimits {
        ...
        cpu_period: config
            ...
            .and_then(Value::as_u64),
        cpu_shares: config
            .pointer("/linux/resources/cpu/shares")
            .and_then(Value::as_u64),
        cpuset_cpus: config
            .pointer("/linux/resources/cpu/cpus")
            .and_then(Value::as_str)
            .map(str::to_string),
        cpuset_mems: config
            .pointer("/linux/resources/cpu/mems")
            .and_then(Value::as_str)
            .map(str::to_string),
        memory_limit_bytes: config
            .pointer("/linux/resources/memory/limit")
            .and_then(Value::as_i64),
        memory_reservation_bytes: config
            .pointer("/linux/resources/memory/reservation")
            .and_then(Value::as_i64),
        memory_swap_bytes: config
            .pointer("/linux/resources/memory/swap")
            .and_then(Value::as_i64),
        pids_limit: config
            .pointer("/linux/resources/pids/limit")
            .and_then(Value::as_i64),
        unified,
    }
}
...
fn enable_controllers(parent: &std::path::Path) -> Result<()> {
    ...
    if !controllers_path.exists() {
        bail!(
            "cgroup v2 is required ({} is missing)",
...
fn create_container_cgroup(id: &str, resources: &ResourceLimits) -> Result<PathBuf> {
    ...
    std::fs::create_dir_all(&path)
        .with_context(|| format!("creating container cgroup {}", path.display()))?;
...
fn attach_guest_network_policy(cgroup_path: &Path) -> Result<()> {
    let cgroup_file = File::open(cgroup_path)
        .with_context(|| format!("opening cgroup {}", cgroup_path.display()))?;
    ...
    if !state.programs_loaded {
        ...
        egress
            .load()
            .context("loading fluxvm_guest_egress into the kernel")?;
        ...
        ingress
            .load()
            .context("loading fluxvm_guest_ingress into the kernel")?;
...
fn attach_guest_lsm() -> Result<()> {
    ...
    for (prog_name, hook) in [
        ...
        let lsm: &mut Lsm = state
            ...
            .try_into()?;
        lsm.load(hook, &btf)
            .with_context(|| format!("loading {prog_name} against lsm/{hook}"))?;
        lsm.attach()
            .with_context(|| format!("attaching {prog_name}"))?;
    }
    state.attached = true;
    Ok(())
}
...
fn configure_container_lsm_policy(
    ...
    if audit_only {
        flags |= FLUXVM_LSM_AUDIT;
    }
    if deny_wx {
        flags |= FLUXVM_LSM_DENY_WX;
    }
    if restrict_writes && !write_prefixes.is_empty() {
        flags |= FLUXVM_LSM_RESTRICT_WRITES;
    }

    let pol_map = state
        .bpf
        .map_mut("fluxvm_lsmpol")
        .context("fluxvm_lsmpol map not found")?;
    let mut pol: aya::maps::HashMap<_, u64, LsmPolicyValue> =
        aya::maps::HashMap::try_from(pol_map)?;
    pol.insert(
        cgroup_id,
        LsmPolicyValue {
            flags,
    ...
    let write_map = state
        .bpf
        .map_mut("fluxvm_lsmwrite")
        .context("fluxvm_lsmwrite map not found")?;
    let mut write: aya::maps::HashMap<_, LsmPrefixKey, LsmPrefixValue> =
        aya::maps::HashMap::try_from(write_map)?;
    for slot in 0..FLUXVM_LSM_MAX_PREFIXES {
        let key = LsmPrefixKey {
            cgroup_id,
            slot,
            reserved: 0,
        };
        match write_prefixes.get(slot as usize) {
            Some(prefix) if prefix.len() < FLUXVM_LSM_PREFIX_LEN => {
                ...
                let value = LsmPrefixValue {
                    len: prefix.len() as u32,
                    bytes,
                };
                write
                    .insert(key, value, 0)
                    .context("writing fluxvm_lsmwrite entry")?;
            }
            _ => {
...
fn forget_container_lsm_policy(cgroup_id: u64) {
    ...
    if let Some(map) = state.bpf.map_mut("fluxvm_lsmwrite") {
        ...
        {
            for slot in 0..FLUXVM_LSM_MAX_PREFIXES {
                let _ = write.remove(&LsmPrefixKey {
                    cgroup_id,
                    slot,
                    reserved: 0,
                });
...
fn cgroup_id_for(path: &Path) -> Result<u64> {
    Ok(std::fs::metadata(path)
        .with_context(|| format!("statting cgroup {}", path.display()))?
        .ino())
}
...
fn apply_resource_limits(path: &std::path::Path, resources: &ResourceLimits) -> Result<()> {
    if resources.cpu_quota.is_some() || resources.cpu_period.is_some() {
        let current =
            std::fs::read_to_string(path.join("cpu.max")).unwrap_or_else(|_| "max 100000".into());
        let mut parts = current.split_whitespace();
        let current_quota = parts.next().unwrap_or("max");
        let current_period = parts
            .next()
    ...
    if let Some(reservation) = resources.memory_reservation_bytes {
        ...
        if !target.exists() {
            bail!("OCI memory.reservation requested but memory.low is unavailable");
        }
    ...
    if let Some(mems) = resources.cpuset_mems.as_deref().filter(|v| !v.is_empty()) {
        ...
        let effective =
            std::fs::read_to_string(parent.join("cpuset.mems.effective")).unwrap_or_default();
...
fn pids_from_cgroup(path: &std::path::Path) -> Result<Vec<u32>> {
    ...
    let text =
        std::fs::read_to_string(&procs).with_context(|| format!("reading {}", procs.display()))?;
    Ok(text
        .lines()
...
fn container_stats(id: &str) -> Result<ContainerStats> {
    ...
    Ok(ContainerStats {
        ...
        pids_limit: if pids_max.trim() == "max" {
            0
        } else {
            pids_max.trim().parse().unwrap_or(0)
        },
    })
}
...
fn parse_process_value(process: &Value, rootfs: PathBuf) -> Result<ProcessSpec> {
    let args = process
        ...
        .map(|v| {
            v.as_str()
                .map(str::to_string)
                .context("process arg must be a string")
        })
        .collect::<Result<Vec<_>>>()?;
    if args.is_empty() {
    ...
    Ok(ProcessSpec {
        ...
        apparmor_profile: process
            .get("apparmorProfile")
            .and_then(Value::as_str)
            .filter(|v| !v.is_empty())
            .map(str::to_string),
        selinux_label: process
            .get("selinuxLabel")
            .and_then(Value::as_str)
            .filter(|v| !v.is_empty())
            .map(str::to_string),
...
fn resolve_executable(spec: &ProcessSpec) -> Result<String> {
    ...
    for dir in path.split(':') {
        let guest = if dir.is_empty() {
            format!("/{arg0}")
        } else {
            format!("{}/{}", dir.trim_end_matches('/'), arg0)
        };
...
fn spawn_gated(
    ...
    let notify_pair = if spec
        .seccomp
        .as_ref()
        .is_some_and(seccomp_profile_uses_notify)
    {
...
mod tests {
    ...
    fn parses_process_security_controls() {
        let (spec, _, _, _) = parse_oci_config(
            r#"{
          "root":{"path":"/run/rootfs"},
          "process":{
            "args":["/bin/sh"],
    ...
    fn rejects_unattached_block_device_nodes() {
        let root =
            std::env::temp_dir().join(format!("fluxvm-block-device-test-{}", std::process::id()));
    ...
    fn formats_selinux_mount_label_like_oci_runtimes() {
        ...
        assert_eq!(
            format_selinux_mount_data(Some("mode=755"), Some(label))
                .unwrap()
                .as_deref(),
    ...
    fn seccomp_notify_broker_enforces_deny_and_continue() {
        ...
        assert_eq!(
            deny,
            -(libc::EACCES as i64),
            "denied syscall should observe the configured errno, got {deny}"
        );
        ...
        assert!(
            cont >= 0,
            "continued syscall should actually execute and succeed, got {cont}"
        );
    }
    ...
    fn run_notify_case(policy: SeccompNotifyPolicy) -> i64 {
        ...
        assert_eq!(
            n, 8,
            "child did not report a getpid() result before exiting"
        );
    ...
    fn oci_swap_translation_matches_cgroup_v2_semantics() {
        assert_eq!(
            oci_swap_to_cgroup2(512, 256).unwrap().as_deref(),
            Some("256")
        );
        assert_eq!(
            oci_swap_to_cgroup2(-1, 256).unwrap().as_deref(),
            Some("max")
        );
        assert_eq!(oci_swap_to_cgroup2(0, 256).unwrap(), None);
        assert_eq!(oci_swap_to_cgroup2(0, -1).unwrap().as_deref(), Some("max"));
        assert_eq!(
            oci_swap_to_cgroup2(512, -1).unwrap().as_deref(),
            Some("512")
        );
    ...
    fn resource_parser_keeps_reservation_swap_and_unified() {
        ...
        assert_eq!(
            resources.unified.get("memory.high").map(String::as_str),
            Some("192")
        );
    }
    ...
    fn guest_cgroup_policy_attaches_and_enforces() {
        ...
        let result = (|| -> Result<()> {
            ...
            assert!(
                deny.is_err(),
                "expected non-loopback connect to be blocked by default-deny policy"
            );
            ...
            assert!(
                allow.is_ok(),
                "expected loopback connect to remain allowed (both this process's egress and, as the listener, its own ingress are in the same policed cgroup)"
            );
    ...
    fn guest_lsm_policy_attaches_and_enforces() {
        ...
        std::fs::create_dir_all(&test_cgroup).expect("creating test cgroup");
        let allowed_dir =
            std::env::temp_dir().join(format!("fluxvm-9s-allowed-{}", std::process::id()));
        std::fs::create_dir_all(&allowed_dir).expect("creating allowed write dir");
        let denied_dir =
            std::env::temp_dir().join(format!("fluxvm-9s-denied-{}", std::process::id()));
        ...
        let result = (|| -> Result<()> {
            ...
            assert!(
                denied.is_err(),
                "expected write outside the declared mount prefix to be denied"
            );
            ...
            assert!(
                allowed.is_ok(),
                "expected write inside the declared mount prefix to remain allowed"
            );
            drop(allowed);
            ...
            assert!(
                rc != 0,
                "expected W+X mprotect to be denied under FLUXVM_LSM_DENY_WX"
            );
            assert_eq!(
    ...
    fn oci_write_policy_extracts_readonly_and_mount_destinations() {
        ...
        assert_eq!(
            prefixes,
            vec!["/data".to_string(), "/etc/hosts".to_string()]
        );
...
mod set8_guest_device_tests {
    ...
    fn rejects_device_path_escape() {
        let err =
            wait_guest_char_device("/dev/../etc/passwd", Duration::from_millis(0)).unwrap_err();
        assert!(
            err.to_string()
