...
pub struct TcpCounters {
    ...
    pub resets: u64,
    pub fins: u64,
    pub flow_alloc_misses: u64,
}
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct TcpLatencyBucket {
    pub kind: String,
    pub bucket: u32,
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct TcpIntelSnapshot {
    pub schema_version: u32,
    pub vm_id: Uuid,
    pub interface: String,
    pub attach_mode: String,
    pub counters: TcpCounters,
    pub handshake_p50_ns: Option<u64>,
    pub handshake_p95_ns: Option<u64>,
    pub rtt_p50_ns: Option<u64>,
    pub rtt_p95_ns: Option<u64>,
    pub latency: Vec<TcpLatencyBucket>,
    pub flows: Vec<TcpFlowRecord>,
...
pub fn attach(
    ...
    pin_root: &Path,
    state_root: &Path,
) -> Result<TcpAttachState> {
    if sample_rate > 1_000_000 {
        bail!("sample_rate must be <= 1000000");
    }
    let _ = read_ifindex(interface)?;
    let pin = vm_pin_dir(pin_root, id);
    let state_path = state_path(state_root, id);
    if state_path.exists() || pin.join("maps/fluxvm_tcp_cfg").exists() {
        bail!("TCP Intelligence already exists for {id}; detach before re-attaching");
    }
    let helper = helper();
    let obj = object();
    let pin_s = pin.display().to_string();
    let key = vm_key(id).to_string();
    let sample = sample_rate.to_string();
    let out = Command::new(&helper)
        .args(["attach", interface, &obj, &pin_s, &key, &sample])
        .output()
...
pub fn read_state(id: Uuid, root: &Path) -> Result<TcpAttachState> {
    let p = state_path(root, id);
    serde_json::from_slice(&fs::read(&p).with_context(|| format!("reading {}", p.display()))?)
        .context("decoding TCP intelligence state")
}
...
pub fn snapshot(
    ...
    state_root: &Path,
    limit: usize,
) -> Result<TcpIntelSnapshot> {
    let state = read_state(id, state_root)?;
    let maps = vm_pin_dir(pin_root, id).join("maps");
    let counters = read_counts(&maps.join("fluxvm_tcp_counts"))?;
    ...
    let rp50 = quantile(&latency, "rtt-estimate", 0.50);
    let rp95 = quantile(&latency, "rtt-estimate", 0.95);
    Ok(TcpIntelSnapshot{schema_version:1,vm_id:id,interface:state.interface,attach_mode:state.attach_mode,counters,handshake_p50_ns:hp50,handshake_p95_ns:hp95,rtt_p50_ns:rp50,rtt_p95_ns:rp95,latency,flows,notes:vec!["handshake response latency is measured from the observed initiating SYN to the opposite-direction SYN/ACK for either guest- or remote-initiated connections".into(),"RTT and retransmission values are passive packet-level estimates, not the guest kernel TCP srtt/retransmission counters".into()]})
}

pub fn stream_events(id: Uuid, pin_root: &Path, seconds: u64, limit: usize) -> Result<()> {
    let map = vm_pin_dir(pin_root, id).join("maps/fluxvm_tcp_events");
    if !map.exists() {
        bail!("TCP event map missing: {}", map.display());
    }
    let h = env::var("FLUXVM_TCP_EVENTS")
        .unwrap_or_else(|_| "/usr/libexec/fluxvm/fluxvm-tcp-events".into());
    let s = Command::new(&h)
        .arg(map)
        ...
        .status()?;
    if !s.success() {
        bail!("TCP event reader exited with {s}");
    }
    Ok(())
}

pub fn prometheus(s: &TcpIntelSnapshot) -> String {
    let id = s.vm_id;
    let mut out = String::new();
    for (name, v) in [
        ("syn", s.counters.syn),
        ("established", s.counters.established),
        ("retransmit", s.counters.retransmits),
        ("rst", s.counters.resets),
        ("fin", s.counters.fins),
        ("flow_alloc_miss", s.counters.flow_alloc_misses),
    ] {
        out.push_str(&format!(
            "fluxvm_tcp_events_total{{vm_id=\"{id}\",event=\"{name}\"}} {v}\n"
        ));
    }
    for (name, v) in [
        ("handshake_p50", s.handshake_p50_ns),
        ("handshake_p95", s.handshake_p95_ns),
        ("rtt_estimate_p50", s.rtt_p50_ns),
        ("rtt_estimate_p95", s.rtt_p95_ns),
    ] {
        if let Some(v) = v {
            out.push_str(&format!(
                "fluxvm_tcp_latency_ns{{vm_id=\"{id}\",quantile=\"{name}\"}} {v}\n"
            ));
        }
    }
    out
...
fn read_counts(map: &Path) -> Result<TcpCounters> {
    ...
    for row in rows(map)? {
        let kind = if let Some(Value::Object(k)) = row.get("key") {
            ...
            let Some(k) = hex(row.get("key")) else {
                continue;
            };
            if k.len() < 4 {
                continue;
        ...
        let value = if let Some(v) = ju(row.get("value")) {
            v
        } else {
        ...
        match kind {
            1 => c.syn = value,
            2 => c.established = value,
            3 => c.retransmits = value,
            4 => c.resets = value,
            5 => c.fins = value,
            6 => c.flow_alloc_misses = value,
            _ => {}
        }
    }
    Ok(c)
...
fn read_hist(map: &Path) -> Result<Vec<TcpLatencyBucket>> {
    ...
    for row in rows(map)? {
        let (kind, bucket) = if let Some(Value::Object(k)) = row.get("key") {
            (
                ju(k.get("kind")).unwrap_or(0) as u32,
                ju(k.get("bucket")).unwrap_or(0) as u32,
            )
        } else {
            let Some(k) = hex(row.get("key")) else {
                continue;
            ...
            (
                u32::from_ne_bytes(k[0..4].try_into().unwrap()),
                u32::from_ne_bytes(k[4..8].try_into().unwrap()),
            )
        };
        let (count, total, max) = if let Some(Value::Object(v)) = row.get("value") {
            (
                ju(v.get("count")).unwrap_or(0),
                ju(v.get("total_ns")).unwrap_or(0),
                ju(v.get("max_ns")).unwrap_or(0),
            )
        } else {
...
fn read_flows(map: &Path) -> Result<Vec<TcpFlowRecord>> {
    ...
    for row in rows(map)? {
            ...
            (row.get("key"), row.get("value"))
        {
            ...
            let Some(guest_bytes) = byte_array(key.get("guest"), 16) else {
                continue;
            };
            let Some(remote_bytes) = byte_array(key.get("remote"), 16) else {
                continue;
            };
        ...
        let Some(value) = hex(row.get("value")) else {
            continue;
        };
        ...
        let u64at =
            |offset: usize| u64::from_ne_bytes(value[offset..offset + 8].try_into().unwrap());
        let u32at =
            |offset: usize| u32::from_ne_bytes(value[offset..offset + 4].try_into().unwrap());
...
fn quantile(v: &[TcpLatencyBucket], kind: &str, q: f64) -> Option<u64> {
    let rows: Vec<&TcpLatencyBucket> = v.iter().filter(|b| b.kind == kind).collect();
    ...
    for b in rows {
        seen += b.count;
        if seen >= target {
            return b.le_ns.or(Some(b.max_ns));
        }
    }
    None
}
fn bound(bucket: u32) -> Option<u64> {
    if bucket >= 24 {
        None
    } else {
...
fn rows(map: &Path) -> Result<Vec<Value>> {
...
fn hex(v: Option<&Value>) -> Option<Vec<u8>> {
    match v? {
        Value::Array(a) => a
            ...
            .collect(),
        Value::Object(o) => o.get("bytes").and_then(|x| hex(Some(x))),
        _ => None,
    }
}
...
fn ju(v: Option<&Value>) -> Option<u64> {
    match v? {
        ...
        Value::Object(o) => o.values().find_map(|x| ju(Some(x))),
        _ => None,
    }
}
fn helper() -> String {
    env::var("FLUXVM_TCP_LOADER").unwrap_or_else(|_| "/usr/libexec/fluxvm/fluxvm-tcp-loader".into())
}
fn object() -> String {
    env::var("FLUXVM_TCP_OBJECT").unwrap_or_else(|_| DEFAULT_OBJECT.into())
}
...
fn write_state(s: &TcpAttachState, root: &Path) -> Result<()> {
    fs::create_dir_all(root)?;
    let p = state_path(root, s.vm_id);
    let tmp = root.join(format!(".{}.{}.tmp", s.vm_id, std::process::id()));
...
mod tests {
    ...
    fn histogram_boundaries() {
        assert_eq!(bound(0), Some(1000));
        assert_eq!(bound(10), Some(1_024_000));
        assert_eq!(bound(24), None);
    }
    #[test]
    fn vm_key_is_stable_for_state() {
        let id = Uuid::nil();
