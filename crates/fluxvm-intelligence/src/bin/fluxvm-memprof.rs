...
#[derive(Deserialize)]
struct MarkRequest { phase: String }
...
async fn main() -> Result<()> {
    ...
    match args.get(1).map(String::as_str) {
        Some("load") => {
            ...
            if args.len() > 3 || (args.len() == 3 && !replace) { bail!(usage()); }
            println!("{}", serde_json::to_string_pretty(&memprof::load(&pin_root, replace)?)?);
            Ok(())
        }
        Some("probe") => {
            println!("{}", serde_json::to_string_pretty(&memprof::probe(&pin_root))?);
            Ok(())
        }
        Some("snapshot") => {
        ...
        Some("mark") => {
            ...
            if args.len() > 4 { bail!(usage()); }
            println!("{}", serde_json::to_string_pretty(&memprof::mark(id, phase, &marker_root)?)?);
            Ok(())
        }
        Some("clear") => {
            let id: Uuid = args.get(2).context(usage())?.parse()?;
            if args.len() > 3 { bail!(usage()); }
        ...
        Some("serve") => {
            let addr: SocketAddr = args.get(2).map(String::as_str).unwrap_or("127.0.0.1:7793").parse()?;
            if args.len() > 3 { bail!(usage()); }
...
async fn serve(addr: SocketAddr, pin_root: PathBuf, marker_root: PathBuf) -> Result<()> {
    let cfg = Config {
        api_url: env::var("FLUXVM_API_URL").unwrap_or_else(|_| "http://127.0.0.1:7788".into()).trim_end_matches('/').to_string(),
        api_token: env::var("FLUXVM_API_TOKEN").ok().filter(|v| !v.is_empty()),
        pin_root,
        marker_root,
        sync_every: Duration::from_millis(env::var("FLUXVM_MEMPROF_SYNC_MS").ok().and_then(|v| v.parse().ok()).unwrap_or(2000)),
    };
    let state = ApiState { cfg: cfg.clone(), snapshots: Arc::new(RwLock::new(BTreeMap::new())) };
    let bg = state.clone();
...
async fn sync_once(state: &ApiState) -> Result<()> {
    ...
    if let Some(token) = &state.cfg.api_token { request = request.bearer_auth(token); }
...
async fn get_vm(State(state): State<ApiState>, AxPath(id): AxPath<Uuid>) -> Response {
    match state.snapshots.read().await.get(&id).cloned() {
        ...
        None => (StatusCode::NOT_FOUND, Json(json!({"error":"memory profile unavailable"}))).into_response(),
    }
}
async fn mark_vm(State(state): State<ApiState>, AxPath(id): AxPath<Uuid>, Json(req): Json<MarkRequest>) -> Response {
    match memprof::mark(id, &req.phase, &state.cfg.marker_root) {
        Ok(v) => Json(json!(v)).into_response(),
        Err(err) => (StatusCode::BAD_REQUEST, Json(json!({"error":format!("{err:#}")}))).into_response(),
    }
}
async fn metrics(State(state): State<ApiState>) -> Response {
    let rows: Vec<_> = state.snapshots.read().await.values().cloned().collect();
    (StatusCode::OK, [("content-type", "text/plain; version=0.0.4; charset=utf-8")], memprof::prometheus(&rows)).into_response()
}
