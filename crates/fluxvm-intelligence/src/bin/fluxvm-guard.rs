...
use fluxvm_intelligence::guard::{self, GuardMode, DEFAULT_GUARD_PIN_ROOT, DEFAULT_GUARD_STATE_ROOT};
...
fn main() -> Result<()> {
    ...
    let pin_root = env::var("FLUXVM_GUARD_PIN_ROOT").map(PathBuf::from).unwrap_or_else(|_| DEFAULT_GUARD_PIN_ROOT.into());
    let state_root = env::var("FLUXVM_GUARD_STATE_ROOT").map(PathBuf::from).unwrap_or_else(|_| DEFAULT_GUARD_STATE_ROOT.into());
    match args.get(1).map(String::as_str) {
        Some("apply") => apply(&args, &pin_root, &state_root),
        Some("status") => {
            let id: Uuid = args.get(2).context(usage())?.parse()?;
            println!("{}", serde_json::to_string_pretty(&guard::status(id, &pin_root, &state_root)?)?);
            Ok(())
        }
        Some("remove") => {
            let id: Uuid = args.get(2).context(usage())?.parse()?;
...
fn apply(args: &[String], pin_root: &std::path::Path, state_root: &std::path::Path) -> Result<()> {
    ...
    while i < args.len() {
        match args[i].as_str() {
            "--mode" => { i += 1; mode = GuardMode::parse(args.get(i).context("--mode requires audit|enforce")?)?; }
            ...
            "--allow-device" => { i += 1; allow_devices.push(PathBuf::from(args.get(i).context("--allow-device requires PATH")?)); }
            "--allow-file" => { i += 1; allow_files.push(PathBuf::from(args.get(i).context("--allow-file requires PATH")?)); }
            other => bail!("unknown option {other:?}\n{}", usage()),
        }
        i += 1;
    }
    let state = guard::apply_policy(id, pid, mode, deny_exec, deny_wx, restrict_devices, restrict_writes, &allow_devices, &allow_files, pin_root, state_root)?;
