...
pub fn load(pin_root: &Path) -> Result<()> {
    ...
    if !out.status.success() {
        bail!("topology loader failed: {}", String::from_utf8_lossy(&out.stderr).trim());
    }
    Ok(())
}
...
pub fn discover_vcpus(pid: u32) -> Result<Vec<VcpuThread>> {
    ...
    for ent in fs::read_dir(&root).with_context(|| format!("reading {}", root.display()))? {
        let ent = ent?;
        let Some(tid) = ent.file_name().to_str().and_then(|s| s.parse::<u32>().ok()) else { continue; };
        let comm = fs::read_to_string(ent.path().join("comm")).unwrap_or_default().trim().to_string();
        let lower = comm.to_ascii_lowercase();
        if lower.contains("vcpu") || lower.contains("cpu ") && lower.contains("kvm") || lower.starts_with("cpu-") {
            found.push((tid, comm));
        }
    }
    found.sort_by_key(|(tid, _)| *tid);
    if found.is_empty() {
        bail!("no vCPU threads recognized under /proc/{pid}/task; refuse to pin arbitrary VMM helper threads");
    }
    Ok(found.into_iter().enumerate().map(|(i, (tid, comm))| VcpuThread { tid, vcpu: i as u32, comm }).collect())
}

pub fn register(id: Uuid, pid: u32, pin_root: &Path) -> Result<Vec<VcpuThread>> {
    let map = pin_root.join("maps/topo_tracked_vcpus");
    if !map.exists() {
        bail!("topology map missing: {}; run fluxvm-topology load first", map.display());
    }
...
pub fn snapshot(id: Uuid, pid: u32, interface: Option<&str>, pin_root: &Path) -> Result<TopologySnapshot> {
    ...
    let interrupts = interface.map(read_interrupts).transpose()?.unwrap_or_default();
    let mut findings = Vec::new();
    for v in &summaries {
        if v.cross_numa {
            findings.push(format!("vCPU {} ran across NUMA nodes {:?}; consider pinning", v.vcpu, v.active_nodes));
        }
        if v.migrations > 100 {
            findings.push(format!("vCPU {} has {} observed scheduler migrations", v.vcpu, v.migrations));
        }
    }
    let irq_total: u64 = irq.iter().map(|s| s.hardirq_ns + s.softirq_ns).sum();
    if irq_total > 100_000_000 {
        findings.push(format!("vCPU-resident CPUs accumulated {:.1} ms of hardirq+softirq time", irq_total as f64 / 1e6));
    }
    if queues.iter().any(|q| q.rps_cpus.as_deref().is_some_and(|v| v == "0" || v.is_empty())) {
        findings.push("one or more RX queues have RPS disabled; a dedicated VM edge may benefit from explicit RPS steering".into());
    }
    let rx:Vec<u64>=queues.iter().filter_map(|q|q.rx_packets).filter(|v|*v>0).collect();
    if rx.len()>1 { let min=*rx.iter().min().unwrap(); let max=*rx.iter().max().unwrap(); if min>0 && max/min>=4 { findings.push(format!("RX queue packet imbalance is at least {}x across observable queues",max/min)); } }
    Ok(TopologySnapshot {
        schema_version: 1,
        vm_id: id,
        vm_key: vm_key(id),
...
pub fn plan(snapshot: &TopologySnapshot) -> Result<SteeringPlan> {
    ...
    let irq_cost: BTreeMap<u32, u64> = snapshot.cpu_irq.iter()
    ...
    if let Some(iface) = &snapshot.interface {
        ...
        for q in &snapshot.queues {
            let qroot = Path::new("/sys/class/net").join(iface).join("queues").join(&q.queue);
    ...
    if let Some(n) = preferred {
        rationale.push(format!("prefer NUMA node {n} using guest-memory locality first, then observed vCPU runtime"));
    }
...
pub fn apply_plan(plan: &SteeringPlan, state_root: &Path) -> Result<SteeringReceipt> {
    ...
    for action in &plan.actions {
        if let Err(err) = apply_action(action, false) {
            ...
            return Err(err.context("steering apply failed; already-applied actions were rolled back best-effort"));
        }
    ...
    atomic_write(&state_root.join(format!("{}.receipt.json", plan.vm_id)), &serde_json::to_vec_pretty(&receipt)?)?;
    Ok(receipt)
}

pub fn rollback(id: Uuid, state_root: &Path) -> Result<()> {
    let path = state_root.join(format!("{id}.receipt.json"));
    let receipt: SteeringReceipt = serde_json::from_slice(&fs::read(&path).with_context(|| format!("reading {}", path.display()))?)?;
...
pub fn prometheus(s: &TopologySnapshot) -> String {
    ...
    for v in &s.vcpus {
        out.push_str(&format!("fluxvm_vcpu_run_ns{{vm_id=\"{}\",vcpu=\"{}\"}} {}\n", s.vm_id, v.vcpu, v.run_ns));
        out.push_str(&format!("fluxvm_vcpu_migrations_total{{vm_id=\"{}\",vcpu=\"{}\"}} {}\n", s.vm_id, v.vcpu, v.migrations));
        out.push_str(&format!("fluxvm_vcpu_cross_numa{{vm_id=\"{}\",vcpu=\"{}\"}} {}\n", s.vm_id, v.vcpu, u8::from(v.cross_numa)));
    }
    for c in &s.cpu_irq {
        out.push_str(&format!("fluxvm_host_irq_ns{{vm_id=\"{}\",cpu=\"{}\",kind=\"hardirq\"}} {}\n", s.vm_id, c.cpu, c.hardirq_ns));
        out.push_str(&format!("fluxvm_host_irq_ns{{vm_id=\"{}\",cpu=\"{}\",kind=\"softirq\"}} {}\n", s.vm_id, c.cpu, c.softirq_ns));
        out.push_str(&format!("fluxvm_host_irq_ns{{vm_id=\"{}\",cpu=\"{}\",kind=\"net-rx\"}} {}\n", s.vm_id, c.cpu, c.net_rx_ns));
        out.push_str(&format!("fluxvm_host_irq_ns{{vm_id=\"{}\",cpu=\"{}\",kind=\"net-tx\"}} {}\n", s.vm_id, c.cpu, c.net_tx_ns));
    }
    out
}
...
fn summarize_vcpus(threads: &[VcpuThread], samples: &[VcpuCpuSample]) -> Vec<VcpuSummary> {
    ...
    for s in samples { by.entry(s.vcpu).or_default().push(s); }
    let keys: BTreeSet<u32> = threads.iter().map(|t| t.vcpu).chain(by.keys().copied()).collect();
    keys.into_iter().map(|vcpu| {
        ...
        let mut active_cpus: Vec<u32> = rows.iter().filter(|r| r.run_ns > 0).map(|r| r.cpu).collect();
        active_cpus.sort_unstable(); active_cpus.dedup();
        let mut active_nodes: Vec<u32> = rows.iter().filter_map(|r| r.numa_node).collect();
        active_nodes.sort_unstable(); active_nodes.dedup();
...
fn choose_node(s: &TopologySnapshot, nodes: &BTreeMap<u32, Vec<u32>>) -> Option<u32> {
    if let Some((&n, _)) = s.memory_pages_by_node.iter().filter(|(n, _)| nodes.contains_key(n)).max_by_key(|(_, pages)| *pages) {
        return Some(n);
    }
    let mut runtime = BTreeMap::<u32, u64>::new();
    for row in &s.vcpu_cpu {
        if let Some(n) = row.numa_node { *runtime.entry(n).or_default() += row.run_ns; }
    }
    runtime.into_iter().max_by_key(|(_, ns)| *ns).map(|(n, _)| n).or_else(|| nodes.keys().next().copied())
}
...
fn read_vcpu_cpu(id: Uuid, pin_root: &Path) -> Result<Vec<VcpuCpuSample>> {
    ...
    for row in dump_rows(&map)? {
        let Some(k) = value_bytes(row.get("key")) else { continue; };
        let Some(v) = value_bytes(row.get("value")) else { continue; };
        if k.len() < 16 || v.len() < 40 { continue; }
        ...
        out.push(VcpuCpuSample {
            ...
            package: read_i32(Path::new(&format!("/sys/devices/system/cpu/cpu{cpu}/topology/physical_package_id"))),
            core: read_i32(Path::new(&format!("/sys/devices/system/cpu/cpu{cpu}/topology/core_id"))),
...
fn read_cpu_irq(pin_root: &Path, cpus: &BTreeSet<u32>) -> Result<Vec<CpuIrqSample>> {
    ...
    for row in dump_rows(&map)? {
        let Some(k) = value_bytes(row.get("key")) else { continue; };
        let Some(v) = value_bytes(row.get("value")) else { continue; };
        if k.len() < 4 || v.len() < 64 { continue; }
        let cpu = u32::from_ne_bytes(k[0..4].try_into().unwrap());
        if !cpus.is_empty() && !cpus.contains(&cpu) { continue; }
        let u = |o: usize| u64::from_ne_bytes(v[o..o+8].try_into().unwrap());
        out.push(CpuIrqSample { cpu, numa_node: cpu_node(cpu), hardirq_count:u(0), hardirq_ns:u(8), hardirq_max_ns:u(16), softirq_count:u(24), softirq_ns:u(32), softirq_max_ns:u(40), net_rx_ns:u(48), net_tx_ns:u(56) });
    }
...
fn read_numa_pages(pid: u32) -> Result<BTreeMap<u32, u64>> {
    ...
    for token in text.split_whitespace() {
        ...
        let Some((n, pages)) = rest.split_once('=') else { continue; };
        let (Ok(n), Ok(pages)) = (n.parse::<u32>(), pages.parse::<u64>()) else { continue; };
...
fn read_queues(iface: &str) -> Result<Vec<QueueState>> {
    ...
    if !root.exists() { bail!("interface {iface} has no sysfs queue directory"); }
    ...
    for ent in fs::read_dir(root)? {
        ...
        let index = queue.split_once('-').and_then(|(_,v)|v.parse::<u32>().ok());
        ...
        out.push(QueueState {
            queue,
            rps_cpus: rps.exists().then(|| fs::read_to_string(rps).unwrap_or_default().trim().to_string()),
            xps_cpus: xps.exists().then(|| fs::read_to_string(xps).unwrap_or_default().trim().to_string()),
            rx_packets,
            tx_packets,
        });
    }
...
fn ethtool_queue_packets(iface:&str)->BTreeMap<(String,u32),u64>{
    let mut out=BTreeMap::new();
    let Ok(result)=Command::new("ethtool").args(["-S",iface]).output() else{return out;};
    ...
    for line in text.lines(){
        ...
        if !lower.contains("packet"){continue;}
        let direction=if lower.contains("rx"){"rx"}else if lower.contains("tx"){"tx"}else{continue;};
        let mut index=None;
        for token in lower.split(|c:char|!c.is_ascii_digit()){if !token.is_empty(){index=token.parse::<u32>().ok();if index.is_some(){break;}}}
        if let Some(q)=index{out.entry((direction.to_string(),q)).and_modify(|v|*v=(*v).max(value)).or_insert(value);}
    }
    out
}
...
fn read_interrupts(iface: &str) -> Result<Vec<InterruptLine>> {
    ...
    for line in text.lines().filter(|line| line.contains(iface)) {
        let Some((irq, _)) = line.split_once(':') else { continue; };
        let Ok(irq) = irq.trim().parse::<u32>() else { continue; };
        let path = PathBuf::from(format!("/proc/irq/{irq}/smp_affinity_list"));
        out.push(InterruptLine { irq, label: line.trim().to_string(), affinity: path.exists().then(|| fs::read_to_string(path).unwrap_or_default().trim().to_string()) });
    }
    Ok(out)
}

fn node_cpus() -> Result<BTreeMap<u32, Vec<u32>>> {
    let online:BTreeSet<u32>=parse_cpu_list(&fs::read_to_string("/sys/devices/system/cpu/online")?)?.into_iter().collect();
    let mut out = BTreeMap::new();
    let root = Path::new("/sys/devices/system/node");
    for ent in fs::read_dir(root)? {
        let ent = ent?;
        let name = ent.file_name().to_string_lossy().to_string();
        let Some(n) = name.strip_prefix("node").and_then(|s| s.parse::<u32>().ok()) else { continue; };
        let mut cpus = parse_cpu_list(&fs::read_to_string(ent.path().join("cpulist"))?)?;
        cpus.retain(|cpu|online.contains(cpu));
        if !cpus.is_empty() { out.insert(n, cpus); }
    }
    if out.is_empty() {
        let cpus:Vec<u32>=online.into_iter().collect();
        if !cpus.is_empty() { out.insert(0, cpus); }
    }
    Ok(out)
}
...
fn parse_cpu_list(input: &str) -> Result<Vec<u32>> {
    ...
    for part in input.trim().split(',').filter(|p| !p.is_empty()) {
        if let Some((a,b)) = part.split_once('-') {
            ...
            if b < a { bail!("invalid CPU range {part}"); }
            out.extend(a..=b);
        } else { out.push(part.parse::<u32>()?); }
    }
...
fn prefer_distinct_cores(cpus:&[u32])->Vec<u32>{
    let mut first=Vec::new(); let mut siblings=Vec::new(); let mut seen=BTreeSet::new();
    for &cpu in cpus{
        let package=read_i32(Path::new(&format!("/sys/devices/system/cpu/cpu{cpu}/topology/physical_package_id"))).unwrap_or(-1);
        let core=read_i32(Path::new(&format!("/sys/devices/system/cpu/cpu{cpu}/topology/core_id"))).unwrap_or(cpu as i32);
        if seen.insert((package,core)){first.push(cpu)}else{siblings.push(cpu)}
    }
    first.extend(siblings); first
}
...
fn cpumask(cpus: &[u32]) -> String {
    ...
    for cpu in cpus { words[*cpu as usize / 32] |= 1u32 << (*cpu % 32); }
    let mut it = words.iter().rev();
    let first = it.next().copied().unwrap_or(0);
    let mut out = format!("{first:x}");
    for word in it { out.push_str(&format!(",{word:08x}")); }
    out
}

fn cpu_list(cpus: &[u32]) -> String { cpus.iter().map(u32::to_string).collect::<Vec<_>>().join(",") }

fn task_affinity(tid: u32) -> Result<String> {
    let out = Command::new("taskset").args(["-pc", &tid.to_string()]).output().context("running taskset")?;
    if !out.status.success() { bail!("taskset query failed for tid {tid}: {}", String::from_utf8_lossy(&out.stderr).trim()); }
    let text = String::from_utf8_lossy(&out.stdout);
    text.rsplit(':').next().map(str::trim).filter(|s| !s.is_empty()).map(str::to_string).ok_or_else(|| anyhow!("unexpected taskset output for tid {tid}: {text}"))
}

fn preflight_action(a: &SteeringAction) -> Result<()> {
    match a {
        SteeringAction::TaskAffinity { tid, old, .. } => {
            let now = task_affinity(*tid)?; if &now != old { bail!("tid {tid} affinity drifted: plan={old}, current={now}"); }
        }
        SteeringAction::SysfsWrite { path, old, .. } | SteeringAction::IrqAffinity { path, old, .. } => {
            let now=fs::read_to_string(path)?.trim().to_string(); if &now != old { bail!("{path} drifted: plan={old}, current={now}"); }
        }
...
fn rollback_preflight(a: &SteeringAction) -> Result<()> {
    match a {
        SteeringAction::TaskAffinity { tid, new, .. } => {
            let now = task_affinity(*tid)?; if &now != new { bail!("refuse rollback: tid {tid} changed since apply: receipt={new}, current={now}"); }
        }
        SteeringAction::SysfsWrite { path, new, .. } | SteeringAction::IrqAffinity { path, new, .. } => {
            let now=fs::read_to_string(path)?.trim().to_string(); if &now != new { bail!("refuse rollback: {path} changed since apply: receipt={new}, current={now}"); }
        }
        SteeringAction::Advisory { .. } => {}
    }
    Ok(())
...
fn apply_action(a: &SteeringAction, reverse: bool) -> Result<()> {
    match a {
        SteeringAction::TaskAffinity { tid, old, new } => {
            let value=if reverse{old}else{new}; let status=Command::new("taskset").args(["-pc",value,&tid.to_string()]).status()?; if !status.success(){bail!("taskset failed for tid {tid}");}
        }
        SteeringAction::SysfsWrite { path, old, new } | SteeringAction::IrqAffinity { path, old, new, .. } => {
...
fn update_tracked_vcpu(map: &Path, tid: u32, key: u64, vcpu: u32, tgid:u32) -> Result<()> {
    let k=tid.to_ne_bytes(); let mut v=Vec::new(); v.extend_from_slice(&key.to_ne_bytes()); v.extend_from_slice(&vcpu.to_ne_bytes()); v.extend_from_slice(&tgid.to_ne_bytes());
    bpftool_update(map,&k,&v)
}

fn remove_vm_tracking(id:Uuid,map:&Path)->Result<()> {
    if !map.exists(){return Ok(());} let wanted=vm_key(id);
    for row in dump_rows(map).unwrap_or_default(){let Some(k)=value_bytes(row.get("key"))else{continue;};let Some(v)=value_bytes(row.get("value"))else{continue;};if k.len()<4||v.len()<16{continue;}if u64::from_ne_bytes(v[0..8].try_into().unwrap())==wanted{let _=delete_hex_key(map,&k[..4]);}}
    Ok(())
}

fn cleanup_stale_tracked(map:&Path)->Result<()> {
    for row in dump_rows(map).unwrap_or_default(){let Some(k)=value_bytes(row.get("key"))else{continue;};let Some(v)=value_bytes(row.get("value"))else{continue;};if k.len()<4||v.len()<16{continue;}let tid=u32::from_ne_bytes(k[0..4].try_into().unwrap());let expected=u32::from_ne_bytes(v[12..16].try_into().unwrap());if read_tgid(tid)!=Some(expected){let _=delete_hex_key(map,&k[..4]);}}
    Ok(())
}

fn read_tgid(tid:u32)->Option<u32>{let text=fs::read_to_string(format!("/proc/{tid}/status")).ok()?;text.lines().find_map(|line|line.strip_prefix("Tgid:").and_then(|v|v.trim().parse().ok()))}

fn bpftool_update(map:&Path,key:&[u8],value:&[u8])->Result<()> {
    let mut cmd=Command::new("bpftool"); cmd.args(["map","update","pinned"]).arg(map).arg("key").arg("hex"); for b in key{cmd.arg(format!("{b:02x}"));} cmd.arg("value").arg("hex"); for b in value{cmd.arg(format!("{b:02x}"));}
    let out=cmd.output()?; if !out.status.success(){bail!("bpftool map update {} failed: {}",map.display(),String::from_utf8_lossy(&out.stderr).trim());} Ok(())
}
fn delete_u32(map:&Path,key:u32)->Result<()> { delete_hex_key(map,&key.to_ne_bytes()) }
fn delete_hex_key(map:&Path,key:&[u8])->Result<()> { let mut cmd=Command::new("bpftool");cmd.args(["map","delete","pinned"]).arg(map).arg("key").arg("hex");for b in key{cmd.arg(format!("{b:02x}"));}let out=cmd.output()?;if !out.status.success(){bail!("bpftool map delete {} failed: {}",map.display(),String::from_utf8_lossy(&out.stderr).trim());}Ok(()) }
fn dump_rows(map:&Path)->Result<Vec<Value>> { let out=Command::new("bpftool").args(["-j","map","dump","pinned"]).arg(map).output()?;if !out.status.success(){bail!("bpftool map dump {} failed: {}",map.display(),String::from_utf8_lossy(&out.stderr).trim());}serde_json::from_slice(&out.stdout).context("decoding bpftool JSON") }
fn value_bytes(v:Option<&Value>)->Option<Vec<u8>> { match v? { Value::Array(a)=>a.iter().map(|x| match x { Value::Number(n)=>n.as_u64().filter(|n|*n<=255).map(|n|n as u8),Value::String(s)=>u8::from_str_radix(s.trim_start_matches("0x"),16).ok(),_=>None }).collect(), Value::Object(o)=>o.get("bytes").and_then(|x|value_bytes(Some(x))), _=>None } }
fn command_exists(name:&str)->bool { Command::new("sh").arg("-c").arg(format!("command -v -- {name} >/dev/null 2>&1")).status().map(|s|s.success()).unwrap_or(false) }
fn read_i32(path:&Path)->Option<i32>{fs::read_to_string(path).ok()?.trim().parse().ok()}
fn unix_seconds()->u64{SystemTime::now().duration_since(UNIX_EPOCH).unwrap_or_default().as_secs()}
fn atomic_write(path:&Path,bytes:&[u8])->Result<()> { if let Some(parent)=path.parent(){fs::create_dir_all(parent)?;}let tmp=path.with_extension(format!("tmp.{}",std::process::id()));fs::write(&tmp,bytes)?;fs::rename(tmp,path)?;Ok(()) }

#[cfg(test)]
mod tests {
    use super::*;
    #[test] fn cpu_list_parser(){assert_eq!(parse_cpu_list("0-2,4,6-7").unwrap(),vec![0,1,2,4,6,7]);}
    #[test] fn cpumask_small(){assert_eq!(cpumask(&[0,1,3]),"b");}
    #[test] fn summary_flags_cross_numa(){let t=vec![VcpuThread{tid:10,vcpu:0,comm:"CPU 0/KVM".into()}];let rows=vec![VcpuCpuSample{vcpu:0,cpu:0,numa_node:Some(0),package:Some(0),core:Some(0),run_ns:10,switches:1,migrations:0,max_slice_ns:10,last_seen_ns:1},VcpuCpuSample{vcpu:0,cpu:8,numa_node:Some(1),package:Some(0),core:Some(0),run_ns:5,switches:1,migrations:1,max_slice_ns:5,last_seen_ns:2}];assert!(summarize_vcpus(&t,&rows)[0].cross_numa);}
}
