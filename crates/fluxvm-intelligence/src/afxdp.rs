...
pub fn probe() -> AfxdpProbe {
    let loader = helper_path(
        "FLUXVM_AFXDP_LOADER",
        "/usr/libexec/fluxvm/fluxvm-afxdp-loader",
    );
    let worker = helper_path(
        "FLUXVM_AFXDP_WORKER",
        "/usr/libexec/fluxvm/fluxvm-afxdp-worker",
    );
    ...
    AfxdpProbe {
        ...
        kernel_af_xdp: Path::new("/sys/module/xsk_diag").exists()
            || Path::new("/proc/net/xdp").exists(),
        notes,
    }
}
...
pub fn build_plan(
    ...
    if !dedicated {
        bail!(
            "refusing AF_XDP fast path without explicit dedicated-interface confirmation (--dedicated)"
        );
    }
    if ifa == ifb {
        bail!("AF_XDP bridge interfaces must be different");
    }
    if qa > MAX_QUEUE_ID || qb > MAX_QUEUE_ID {
        bail!("AF_XDP queue id must be 0..={MAX_QUEUE_ID}");
    }
    let a = inspect_queue(ifa, qa)?;
    let b = inspect_queue(ifb, qb)?;
    if a.mtu > MAX_SUPPORTED_MTU || b.mtu > MAX_SUPPORTED_MTU {
        bail!(
            "Set 9 does not forward AF_XDP multi-buffer packets; MTU must be <= {MAX_SUPPORTED_MTU} ({}={}, {}={})",
            ifa,
            a.mtu,
            ifb,
            b.mtu
    ...
    if let Some(m) = &a.master {
        warnings.push(format!(
            "{ifa} is enslaved to {m}; dedicated ownership must include that topology"
        ));
    }
    if let Some(m) = &b.master {
        warnings.push(format!(
            "{ifb} is enslaved to {m}; dedicated ownership must include that topology"
        ));
    }
...
pub fn start(plan: &AfxdpPlan, pin_base: &Path, state_root: &Path) -> Result<AfxdpControl> {
    ...
    if control_path.exists() {
        ...
        if process_alive(old.worker_pid) {
            bail!(
                "AF_XDP fast path already active for {} with pid {}",
                plan.vm_id,
                old.worker_pid
            );
        }
        bail!(
            "stale AF_XDP control state exists at {}; run stop before start",
    ...
    let object = env::var("FLUXVM_AFXDP_OBJECT")
        .unwrap_or_else(|_| "/usr/lib/fluxvm/bpf/fluxvm_afxdp.bpf.o".into());
    ...
    let mut child = Command::new(&worker)
        ...
        .spawn()
        .with_context(|| format!("starting {worker}"))?;
    thread::sleep(Duration::from_millis(250));
    if let Some(status) = child.try_wait()? {
        let _ = unload(&loader, &pin_root, plan);
        bail!(
            "AF_XDP worker exited during startup with {status}; see {}",
            log_path.display()
        );
    }
...
pub fn stop(id: Uuid, state_root: &Path) -> Result<()> {
    ...
    let control: AfxdpControl = serde_json::from_slice(
        &fs::read(&path).with_context(|| format!("reading {}", path.display()))?,
    )?;
    if process_alive(control.worker_pid) {
        let _ = Command::new("kill")
            .args(["-TERM", &control.worker_pid.to_string()])
            .status();
        for _ in 0..20 {
            if !process_alive(control.worker_pid) {
                break;
        ...
        if process_alive(control.worker_pid) {
            let _ = Command::new("kill")
                .args(["-KILL", &control.worker_pid.to_string()])
                .status();
        }
    }
    let loader = helper_path(
...
pub fn status(id: Uuid, state_root: &Path) -> Result<AfxdpStatus> {
    let path = control_path(state_root, id);
    let control: Option<AfxdpControl> = if path.exists() {
    ...
    let (running, worker_pid, loader, runtime, xdp) = if let Some(c) = &control {
        ...
        if !running {
            findings.push("worker is not alive; XSK removal makes the BPF redirect path fall back to XDP_PASS".into());
        }
        ...
        let out = Command::new(helper)
            .arg("status")
            .arg(&c.pin_root)
            .arg(&c.plan.side_a.interface)
            .arg(&c.plan.side_b.interface)
            .output();
        let loader = out
            .ok()
        ...
        (
            ...
            Some(c.worker_pid),
            loader,
            read_runtime(root).unwrap_or_default(),
            read_xdp_stats(root).unwrap_or_default(),
        )
    } else {
    ...
    for r in &runtime {
        if r.tx_ring_full > 0 {
            findings.push(format!(
                "slot {} queue {} observed {} TX-ring-full drops",
                r.slot, r.queue, r.tx_ring_full
            ));
        }
        if r.multibuf_drops > 0 {
            findings.push(format!("slot {} queue {} dropped {} multi-buffer descriptors; reduce MTU or use the kernel datapath", r.slot, r.queue, r.multibuf_drops));
        }
        if !r.zero_copy {
            findings.push(format!(
                "slot {} queue {} is running AF_XDP copy mode",
                r.slot, r.queue
            ));
        }
    }
...
pub fn list_statuses(state_root: &Path) -> Result<Vec<AfxdpStatus>> {
    ...
    ids.sort();
    ids.into_iter().map(|id| status(id, state_root)).collect()
}
...
pub fn prometheus(s: &AfxdpStatus) -> String {
    ...
    out.push_str(&format!(
        "fluxvm_afxdp_running{{vm_id=\"{id}\"}} {}\n",
        if s.running { 1 } else { 0 }
    ));
    for r in &s.runtime {
        let l = format!("vm_id=\"{id}\",slot=\"{}\",queue=\"{}\"", r.slot, r.queue);
        out.push_str(&format!(
            "fluxvm_afxdp_rx_packets{{{l}}} {}\n",
            r.rx_packets
        ));
        out.push_str(&format!(
            "fluxvm_afxdp_tx_packets{{{l}}} {}\n",
            r.tx_packets
        ));
        out.push_str(&format!(
            "fluxvm_afxdp_dropped_packets{{{l}}} {}\n",
            r.dropped_packets
        ));
        out.push_str(&format!(
            "fluxvm_afxdp_zero_copy{{{l}}} {}\n",
    ...
    for x in &s.xdp {
        ...
        out.push_str(&format!(
            "fluxvm_afxdp_xdp_seen_packets{{{l}}} {}\n",
            x.seen_packets
        ));
        out.push_str(&format!(
            "fluxvm_afxdp_xdp_redirect_attempts{{{l}}} {}\n",
            x.redirect_attempts
        ));
    }
    out
...
pub fn stream_events(id: Uuid, seconds: u64, limit: usize, state_root: &Path) -> Result<()> {
    ...
    let helper = helper_path(
        "FLUXVM_AFXDP_EVENTS",
        "/usr/libexec/fluxvm/fluxvm-afxdp-events",
    );
    let status = Command::new(helper)
        .arg(&control.pin_root)
        .arg(seconds.to_string())
        .arg(limit.to_string())
        .status()?;
    if !status.success() {
        bail!("AF_XDP event reader failed with {status}");
    }
    Ok(())
}
...
fn inspect_queue(iface: &str, queue: u32) -> Result<InterfaceQueue> {
    ...
    if !q.exists() {
        bail!("interface {iface} has no RX queue {queue}");
    }
    let ifindex = read_u32(root.join("ifindex"))?;
    let mtu = read_u32(root.join("mtu"))?;
    let master = fs::read_link(root.join("master"))
        .ok()
...
fn validate_plan_live(plan: &AfxdpPlan) -> Result<()> {
    if plan.schema_version != AFXDP_SCHEMA_VERSION {
        bail!("unsupported AF_XDP plan schema {}", plan.schema_version);
    }
    if !plan.dedicated_interfaces_confirmed {
        bail!("plan lacks dedicated-interface confirmation");
    }
    let a = inspect_queue(&plan.side_a.interface, plan.side_a.queue)?;
    let b = inspect_queue(&plan.side_b.interface, plan.side_b.queue)?;
    if a.ifindex != plan.side_a.ifindex || b.ifindex != plan.side_b.ifindex {
        bail!("interface identity drift since plan generation; regenerate plan");
    }
    if a.mtu != plan.side_a.mtu || b.mtu != plan.side_b.mtu {
        bail!("interface MTU drift since plan generation; regenerate plan");
    }
    Ok(())
}

fn read_runtime(pin_root: &Path) -> Result<Vec<QueueRuntime>> {
    let rows = dump_map(&pin_root.join("maps/afxdp_runtime"))?;
    let mut out = Vec::new();
    for row in rows {
        let Some(k) = bytes(row.get("key")) else {
            continue;
        };
        let Some(v) = bytes(row.get("value")) else {
            continue;
        };
        if k.len() < 4 || v.len() < 88 {
            continue;
        }
        let key = u32::from_ne_bytes(k[0..4].try_into().unwrap());
        let u64at = |o: usize| u64::from_ne_bytes(v[o..o + 8].try_into().unwrap());
        let u32at = |o: usize| u32::from_ne_bytes(v[o..o + 4].try_into().unwrap());
        let worker = u32at(84);
        if worker == 0 {
            continue;
        }
        out.push(QueueRuntime {
            slot: key / 64,
            queue: key % 64,
            rx_packets: u64at(0),
            rx_bytes: u64at(8),
            tx_packets: u64at(16),
            tx_bytes: u64at(24),
            dropped_packets: u64at(32),
            tx_ring_full: u64at(40),
            fill_deferred: u64at(48),
            poll_wakeups: u64at(56),
            multibuf_drops: u64at(64),
            last_update_ns: u64at(72),
            zero_copy: u32at(80) != 0,
            worker_pid: worker,
        });
    }
    out.sort_by_key(|r| (r.slot, r.queue));
    Ok(out)
}

fn read_xdp_stats(pin_root: &Path) -> Result<Vec<XdpQueueStat>> {
    let rows = dump_map(&pin_root.join("maps/afxdp_xdp_stats"))?;
    let mut out = Vec::new();
    for row in rows {
        let Some(k) = bytes(row.get("key")) else {
            continue;
        ...
        out.push(XdpQueueStat {
            slot: key / 64,
            queue: key % 64,
            seen_packets: sums[0],
            seen_bytes: sums[1],
            redirect_attempts: sums[2],
            pass_inactive: sums[3],
            pass_queue_disabled: sums[4],
        });
    }
    out.sort_by_key(|r| (r.slot, r.queue));
    Ok(out)
...
fn validate_iface_name(name: &str) -> Result<()> {
    if name.is_empty() || name.len() > 15 || name.contains('/') || name.contains('\0') {
...
fn command_exists(cmd: &str) -> bool {
    if cmd.contains('/') {
        ...
        Command::new("sh")
            .args([
                "-c",
                &format!("command -v -- {} >/dev/null 2>&1", shell_word(cmd)),
            ])
            .status()
            .map(|s| s.success())
            .unwrap_or(false)
...
mod tests {
    ...
    #[test]
    fn modes() {
        assert_eq!(AfxdpMode::parse("zerocopy").unwrap(), AfxdpMode::Zerocopy);
        assert!(AfxdpMode::parse("magic").is_err());
    }
    #[test]
    fn metrics_are_labeled() {
        let s = AfxdpStatus {
            schema_version: 1,
            vm_id: Uuid::nil(),
            running: false,
        ...
        assert!(prometheus(&s).contains("fluxvm_afxdp_running"));
    }
    #[test]
    fn key_math() {
        let key = 1 * 64 + 63;
