...
use fluxvm_intelligence::{
    DEFAULT_PIN_ROOT, FeatureProbe, FlightRecorderSnapshot, NetworkVmStats, VmDiagnosis,
    VmRuntimeSnapshot, diagnose_vm_with_reasons, probe, register_raw, snapshot_raw,
    snapshot_record, trace_events, unregister_exact, unregister_raw, unregister_stale_tids,
};
use fluxvm_network::{
...
impl Config {
    fn from_env() -> Result<Self> {
        Ok(Self {
            ...
            listen: env::var("FLUXVM_INTEL_LISTEN")
                .unwrap_or_else(|_| "127.0.0.1:7790".into())
                .parse()
                .context("FLUXVM_INTEL_LISTEN")?,
            pin_root: env::var("FLUXVM_INTEL_PIN_ROOT")
                .map(PathBuf::from)
                .unwrap_or_else(|_| DEFAULT_PIN_ROOT.into()),
            sync_every: Duration::from_millis(
                env::var("FLUXVM_INTEL_SYNC_MS")
                    .ok()
...
async fn main() -> Result<()> {
    ...
    match args.get(1).map(String::as_str).unwrap_or("daemon") {
        "probe" => {
            println!("{}", serde_json::to_string_pretty(&probe(&cfg.pin_root))?);
            Ok(())
        }
        "register" => {
            let (id, pid) = parse_id_pid(&args)?;
            let tids = register_raw(id, pid, &cfg.pin_root)?;
            println!("{}", json!({"vm_id":id,"pid":pid,"tracked_tids":tids}));
            Ok(())
        }
        "unregister" => {
            let id = args
                .get(2)
                .context("usage: fluxvm-intelligence unregister <uuid> [pid]")?
                .parse()?;
            let pid = args.get(3).and_then(|s| s.parse().ok());
            unregister_raw(id, pid, &cfg.pin_root)?;
            Ok(())
        }
        "snapshot" => {
            let (id, pid) = parse_id_pid(&args)?;
            println!(
                "{}",
                serde_json::to_string_pretty(&snapshot_raw(id, pid, &cfg.pin_root)?)?
            );
            Ok(())
        }
        ...
        other => bail!(
            "unknown command {other}; use daemon|probe|register|unregister|snapshot|diagnose|trace"
        ),
    }
}
...
async fn daemon(cfg: Config) -> Result<()> {
    ...
    tokio::spawn(async move {
        loop {
            if let Err(e) = sync_once(&bg_cfg, &bg_state).await {
                warn!(error=%e,"runtime intelligence sync failed");
            }
            tokio::time::sleep(bg_cfg.sync_every).await;
        }
    });
    let app = Router::new()
...
async fn sync_once(cfg: &Config, state: &AppState) -> Result<()> {
    ...
    let mut req = client.get(format!("{}/v1/vms", cfg.api_url));
            ...
            Ok(mut s) => {
                ...
                if let Some((old_pid, old_tids)) = old_regs.get(&vm.id) {
                    if *old_pid != pid {
                        ...
                        let stale: Vec<u32> = old_tids
                            .iter()
                            .copied()
                            .filter(|t| !now.contains(t))
                            .collect();
                        let _ = unregister_stale_tids(&stale, &cfg.pin_root);
                    }
                }
                next_regs.insert(vm.id, (pid, tids));
                next.insert(vm.id, s);
            }
    ...
    for (id, (pid, tids)) in &old_regs {
        if !next_regs.contains_key(id) {
            let _ = unregister_exact(*id, *pid, tids, &cfg.pin_root);
        }
    }
...
async fn diagnose(Path(id): Path<Uuid>, State(state): State<AppState>) -> Response {
    let Some(snapshot) = state.snapshots.read().await.get(&id).cloned() else {
        return (
            ...
            Json(json!({"error":"VM intelligence snapshot not found"})),
        )
            .into_response();
    };
    let policy = match fetch_network_policy(&state.cfg, id).await {
        ...
        Err(e) => {
            return (
                ...
                Json(json!({"error":format!("network policy: {e:#}")})),
            )
                .into_response();
        }
    };
    let pod_policy = match fetch_pod_policy(&state.cfg, id).await {
        ...
        Err(e) => {
            return (
                StatusCode::BAD_GATEWAY,
                Json(json!({"error":format!("pod policy: {e:#}")})),
            )
                .into_response();
        }
    };
    let flows = match fetch_network_flows(&state.cfg, id, 256).await {
        ...
        Err(e) => {
            return (
                StatusCode::BAD_GATEWAY,
                Json(json!({"error":format!("network flows: {e:#}")})),
            )
                .into_response();
        }
    };
    ...
    let reasons = fetch_drop_reasons(&state.cfg, id, 256)
        .await
        .unwrap_or_default();
    let report: VmDiagnosis =
        diagnose_vm_with_reasons(&snapshot, &policy, pod_policy.as_ref(), &flows, &reasons);
...
async fn fetch_network_policy(cfg: &Config, id: Uuid) -> Result<VmNetworkPolicy> {
    let response = authed_get(
        cfg,
        format!("{}/v1/vms/{}/network/effective", cfg.api_url, id),
    )
    .send()
    .await?
    .error_for_status()?;
...
async fn fetch_pod_policy(cfg: &Config, id: Uuid) -> Result<Option<PodNetworkPolicy>> {
    let response = authed_get(
        cfg,
        format!("{}/v1/vms/{}/network/pod-policy", cfg.api_url, id),
    )
    .send()
    .await?;
...
async fn fetch_drop_reasons(cfg: &Config, id: Uuid, limit: usize) -> Result<Vec<DropReasonRecord>> {
    let response = authed_get(
        cfg,
        format!(
            "{}/v1/vms/{}/network/drop-reasons?limit={}",
            cfg.api_url,
            id,
            limit.clamp(1, 4096)
        ),
...
async fn status(State(s): State<AppState>) -> Json<Status> {
    let p = s.probe.read().await.clone();
    let n = s.snapshots.read().await.len();
    Json(Status {
        ok: p.ready_for_scheduler(),
        probe: p,
        vm_count: n,
    })
}
async fn list_vms(State(s): State<AppState>) -> Json<Vec<VmRuntimeSnapshot>> {
    Json(s.snapshots.read().await.values().cloned().collect())
}
async fn get_vm(Path(id): Path<Uuid>, State(s): State<AppState>) -> Response {
    match s.snapshots.read().await.get(&id).cloned() {
...
async fn flight(Path(id): Path<Uuid>, State(s): State<AppState>) -> Response {
    match s
        ...
        .await
        .get(&id)
        .and_then(|v| v.flight.clone())
    {
        Some(v) => Json(v).into_response(),
        None => (
            StatusCode::NOT_FOUND,
            Json(json!({"error":"Flight Recorder snapshot unavailable"})),
        )
            .into_response(),
    }
}
async fn metrics(State(s): State<AppState>) -> Response {
    ...
    for v in rows.values() {
        ...
        out.push_str(&format!(
            "fluxvm_intel_kvm_exits_total{{{labels}}} {}\n",
            v.kernel.kvm_exits
        ));
        out.push_str(&format!(
            "fluxvm_intel_guest_run_seconds_total{{{labels}}} {:.9}\n",
            v.kernel.guest_run_ns as f64 / 1e9
        ));
        out.push_str(&format!(
            "fluxvm_intel_sched_wakeups_total{{{labels}}} {}\n",
            v.kernel.sched_wakeups
        ));
        out.push_str(&format!(
            "fluxvm_intel_runnable_delay_seconds_total{{{labels}}} {:.9}\n",
            v.kernel.runnable_delay_ns as f64 / 1e9
        ));
        out.push_str(&format!(
            "fluxvm_intel_runnable_delay_max_seconds{{{labels}}} {:.9}\n",
            v.kernel.runnable_delay_max_ns as f64 / 1e9
        ));
        out.push_str(&format!(
            "fluxvm_intel_thread_migrations_total{{{labels}}} {}\n",
            v.kernel.migrations
        ));
        out.push_str(&format!(
            "fluxvm_intel_vmm_read_bytes{{{labels}}} {}\n",
            v.process.read_bytes
        ));
        out.push_str(&format!(
            "fluxvm_intel_vmm_write_bytes{{{labels}}} {}\n",
            v.process.write_bytes
        ));
        if let Some(n) = &v.network {
            out.push_str(&format!(
                "fluxvm_intel_network_allowed_packets_total{{{labels}}} {}\n",
                n.allowed_packets
            ));
            out.push_str(&format!(
                "fluxvm_intel_network_dropped_packets_total{{{labels}}} {}\n",
                n.dropped_packets
            ));
            out.push_str(&format!(
                "fluxvm_intel_network_dropped_bytes_total{{{labels}}} {}\n",
                n.dropped_bytes
            ));
        }
        if let Some(f) = &v.flight {
            append_flight_metrics(&mut out, &labels, f);
        }
    }
    (
        StatusCode::OK,
...
fn append_flight_metrics(out: &mut String, labels: &str, flight: &FlightRecorderSnapshot) {
    ...
    out.push_str(&format!(
        "fluxvm_intel_block_started_total{{{labels}}} {}\n",
        flight.counters.block_started
    ));
    out.push_str(&format!(
        "fluxvm_intel_block_completed_total{{{labels}}} {}\n",
        flight.counters.block_completed
    ));
    out.push_str(&format!(
        "fluxvm_intel_block_orphan_completions_total{{{labels}}} {}\n",
        flight.counters.block_orphan_completions
    ));
    out.push_str(&format!(
        "fluxvm_intel_vhost_queued_total{{{labels}}} {}\n",
        flight.counters.vhost_queued
    ));
    out.push_str(&format!(
        "fluxvm_intel_vhost_wakeups_total{{{labels}}} {}\n",
        flight.counters.vhost_wakeups
    ));
    out.push_str(&format!(
        "fluxvm_intel_flight_events_lost_total{{{labels}}} {}\n",
    ...
    for kind in ["kvm-run", "runnable", "block-io"] {
        ...
        for (bucket, count) in counts.iter().enumerate() {
            ...
            let le = if bucket >= 24 {
                ...
                format!(
                    "{:.9}",
                    (1000u64.saturating_mul(1u64 << bucket)) as f64 / 1e9
                )
            };
            out.push_str(&format!("fluxvm_intel_latency_seconds_bucket{{{labels},kind=\"{kind}\",le=\"{le}\"}} {cumulative}\n"));
        }
        out.push_str(&format!(
            "fluxvm_intel_latency_seconds_count{{{labels},kind=\"{kind}\"}} {cumulative}\n"
        ));
        out.push_str(&format!(
            "fluxvm_intel_latency_seconds_sum{{{labels},kind=\"{kind}\"}} {:.9}\n",
            total_ns as f64 / 1e9
        ));
    }
}
