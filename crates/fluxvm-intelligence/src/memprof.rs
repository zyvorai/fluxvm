...
pub fn probe(pin_root: &Path) -> MemprofProbe {
    ...
    MemprofProbe {
        ...
        direct_reclaim_link: links.join("reclaim_begin").exists() && links.join("reclaim_end").exists(),
...
pub fn load(pin_root: &Path, replace_links: bool) -> Result<Value> {
    let helper = helper_path("FLUXVM_MEMPROF_LOADER", "/usr/libexec/fluxvm/fluxvm-memprof-loader", "fluxvm-memprof-loader");
    ...
    if replace_links { cmd.arg("--replace-links"); }
    let out = cmd.output().with_context(|| format!("running {}", helper.display()))?;
    if !out.status.success() {
        bail!("memprof loader failed: {}", String::from_utf8_lossy(&out.stderr).trim());
    }
    serde_json::from_slice(&out.stdout).context("decoding memprof loader response")
}

pub fn snapshot_record(record: &VmRecord, pin_root: &Path, marker_root: &Path) -> Result<MemoryProfileSnapshot> {
    ...
    snapshot(record.id, pid, record.cgroup_path.as_deref(), pin_root, marker_root)
}

pub fn snapshot_raw(id: Uuid, pid: u32, cgroup: Option<&Path>, pin_root: &Path, marker_root: &Path) -> Result<MemoryProfileSnapshot> {
    if probe(pin_root).shared_runtime_maps {
...
pub fn snapshot(id: Uuid, pid: u32, cgroup: Option<&Path>, pin_root: &Path, marker_root: &Path) -> Result<MemoryProfileSnapshot> {
    if !Path::new(&format!("/proc/{pid}")).exists() {
        bail!("pid {pid} does not exist");
    }
    let p = probe(pin_root);
    let key = vm_key(id);
    let kernel = if p.maps_loaded { read_kernel(pin_root, key).unwrap_or_default() } else { KernelMemoryStats::default() };
    let histogram = if p.maps_loaded { read_histogram(pin_root, key).unwrap_or_default() } else { Vec::new() };
    let first = if p.maps_loaded { read_first(pin_root, key).unwrap_or_default() } else { KernelBootFirst::default() };
    ...
    if kernel.major_faults > 0 {
        findings.push(format!("{} eBPF-attributed major page fault(s)", kernel.major_faults));
    }
    if kernel.fault_max_ns >= 5_000_000 {
        findings.push(format!("slow page-fault path: {:.2} ms max", kernel.fault_max_ns as f64 / 1e6));
    }
    if kernel.reclaim_max_ns >= 20_000_000 {
        findings.push(format!("slow direct reclaim: {:.2} ms max", kernel.reclaim_max_ns as f64 / 1e6));
    }
    if kernel.ringbuf_lost > 0 {
        findings.push(format!("{} memory-profiler live event(s) lost to ring-buffer pressure", kernel.ringbuf_lost));
    }
    if cgroup_stats.events_high > 0 {
        findings.push(format!("memory.high throttling observed {} time(s)", cgroup_stats.events_high));
    }
    if cgroup_stats.events_oom > 0 {
        findings.push(format!("cgroup OOM observed {} time(s)", cgroup_stats.events_oom));
    }
    if cgroup_stats.events_oom_kill > 0 {
        findings.push(format!("cgroup OOM killed {} task(s)", cgroup_stats.events_oom_kill));
    }
    if cgroup_stats.psi_full_avg10.unwrap_or(0.0) >= 1.0 {
        findings.push(format!("full memory PSI avg10 is {:.2}%", cgroup_stats.psi_full_avg10.unwrap_or(0.0)));
    }
    if let (Some(current), Some(max)) = (cgroup_stats.current_bytes, cgroup_stats.max_bytes) {
        if max > 0 && current.saturating_mul(100) >= max.saturating_mul(90) {
            findings.push(format!("memory.current is {:.1}% of memory.max", current as f64 * 100.0 / max as f64));
        }
    }
    ...
    if p.maps_loaded && !p.page_fault_link {
        findings.push("handle_mm_fault probes unavailable; kernel fault-latency samples are disabled".into());
    }
...
pub fn mark(id: Uuid, phase: &str, marker_root: &Path) -> Result<MarkerState> {
    ...
    fs::create_dir_all(marker_root).with_context(|| format!("creating {}", marker_root.display()))?;
    ...
    fs::write(&tmp, serde_json::to_vec_pretty(&state)?).with_context(|| format!("writing {}", tmp.display()))?;
...
pub fn clear_markers(id: Uuid, marker_root: &Path) -> Result<()> {
    let path = marker_path(id, marker_root);
    if path.exists() { fs::remove_file(&path).with_context(|| format!("removing {}", path.display()))?; }
    Ok(())
}

pub fn events(id: Uuid, pin_root: &Path, seconds: u64, limit: usize) -> Result<Vec<MemoryEvent>> {
    let helper = helper_path("FLUXVM_MEMPROF_EVENTS", "/usr/libexec/fluxvm/fluxvm-memprof-events", "fluxvm-memprof-events");
    let pin = mem_map(pin_root, "memprof_events");
    if !pin.exists() { bail!("{} is not loaded", pin.display()); }
    ...
    if !out.status.success() {
        bail!("memprof event reader failed: {}", String::from_utf8_lossy(&out.stderr).trim());
    }
...
pub fn prometheus(rows: &[MemoryProfileSnapshot]) -> String {
    ...
    for row in rows {
        ...
        metric_opt_u64(&mut out, "fluxvm_memprof_cgroup_memory_current_bytes", &labels, row.cgroup.current_bytes);
        metric_opt_u64(&mut out, "fluxvm_memprof_cgroup_memory_peak_bytes", &labels, row.cgroup.peak_bytes);
        metric_opt_u64(&mut out, "fluxvm_memprof_cgroup_memory_max_bytes", &labels, row.cgroup.max_bytes);
        out.push_str(&format!("fluxvm_memprof_faults_total{{{labels}}} {}\n", row.kernel.faults));
        out.push_str(&format!("fluxvm_memprof_major_faults_total{{{labels}}} {}\n", row.kernel.major_faults));
        out.push_str(&format!("fluxvm_memprof_direct_reclaim_total{{{labels}}} {}\n", row.kernel.reclaim_events));
        out.push_str(&format!("fluxvm_memprof_events_oom_total{{{labels}}} {}\n", row.cgroup.events_oom));
        out.push_str(&format!("fluxvm_memprof_events_oom_kill_total{{{labels}}} {}\n", row.cgroup.events_oom_kill));
        out.push_str(&format!("fluxvm_memprof_events_high_total{{{labels}}} {}\n", row.cgroup.events_high));
        if let Some(v) = row.cgroup.psi_some_avg10 { out.push_str(&format!("fluxvm_memprof_memory_psi_some_avg10{{{labels}}} {v}\n")); }
        if let Some(v) = row.cgroup.psi_full_avg10 { out.push_str(&format!("fluxvm_memprof_memory_psi_full_avg10{{{labels}}} {v}\n")); }
        metric_opt_f64(&mut out, "fluxvm_memprof_process_to_first_kvm_seconds", &labels, row.boot.process_to_first_kvm_ms.map(|v| v / 1000.0));
        metric_opt_f64(&mut out, "fluxvm_memprof_process_to_first_vhost_seconds", &labels, row.boot.process_to_first_vhost_ms.map(|v| v / 1000.0));
        metric_opt_f64(&mut out, "fluxvm_memprof_process_to_guest_ready_seconds", &labels, row.boot.process_to_guest_ready_ms.map(|v| v / 1000.0));
        metric_opt_f64(&mut out, "fluxvm_memprof_snapshot_seconds", &labels, row.boot.snapshot_ms.map(|v| v / 1000.0));
        metric_opt_f64(&mut out, "fluxvm_memprof_restore_seconds", &labels, row.boot.restore_ms.map(|v| v / 1000.0));
        append_histogram(&mut out, &labels, &row.histogram, "page-fault", "fluxvm_memprof_page_fault_seconds");
        append_histogram(&mut out, &labels, &row.histogram, "direct-reclaim", "fluxvm_memprof_direct_reclaim_seconds");
    }
    out
}

fn append_histogram(out: &mut String, labels: &str, rows: &[MemoryHistogramBucket], kind: &str, metric: &str) {
    let mut counts = [0u64; HIST_BUCKETS];
    let mut sum_ns = 0u64;
    for row in rows.iter().filter(|r| r.kind == kind) {
        if let Some(slot) = counts.get_mut(row.bucket as usize) { *slot = slot.saturating_add(row.count); }
        sum_ns = sum_ns.saturating_add(row.total_ns);
    }
    let mut cumulative = 0u64;
    for (bucket, count) in counts.iter().enumerate() {
        cumulative = cumulative.saturating_add(*count);
        let le = if bucket + 1 == HIST_BUCKETS { "+Inf".to_string() } else { format!("{:.9}", bucket_le_ns(bucket as u32).unwrap_or(0) as f64 / 1e9) };
        out.push_str(&format!("{metric}_bucket{{{labels},le=\"{le}\"}} {cumulative}\n"));
    }
    out.push_str(&format!("{metric}_count{{{labels}}} {cumulative}\n"));
    out.push_str(&format!("{metric}_sum{{{labels}}} {:.9}\n", sum_ns as f64 / 1e9));
}

fn metric_opt_u64(out: &mut String, name: &str, labels: &str, value: Option<u64>) {
    if let Some(v) = value { out.push_str(&format!("{name}{{{labels}}} {v}\n")); }
}
fn metric_opt_f64(out: &mut String, name: &str, labels: &str, value: Option<f64>) {
    if let Some(v) = value { out.push_str(&format!("{name}{{{labels}}} {v}\n")); }
}

fn boot_profile(start: u64, ticks: u64, kernel: KernelBootFirst, markers: BTreeMap<String, u64>) -> BootProfile {
    ...
    BootProfile {
        ...
        pause_control_ms: delta_ms(markers.get("pause-request").copied(), markers.get("paused").copied()),
        resume_control_ms: delta_ms(markers.get("resume-request").copied(), markers.get("resumed").copied()),
        snapshot_ms: delta_ms(markers.get("snapshot-begin").copied(), markers.get("snapshot-end").copied()),
        restore_ms: delta_ms(markers.get("restore-begin").copied(), markers.get("restore-end").copied()),
        kernel,
        markers_ns: markers,
    }
}
...
fn read_kernel(pin_root: &Path, wanted: u64) -> Result<KernelMemoryStats> {
    for row in bpftool_rows(&mem_map(pin_root, "memprof_stats"))? {
        ...
        if read_u64(&key, 0)? != wanted { continue; }
        let value = bytes(row.get("value"))?;
        if value.len() < 64 { bail!("memprof_stats value is {} bytes, expected 64", value.len()); }
...
fn read_histogram(pin_root: &Path, wanted: u64) -> Result<Vec<MemoryHistogramBucket>> {
    ...
    for row in bpftool_rows(&mem_map(pin_root, "memprof_hist"))? {
        let key = bytes(row.get("key"))?;
        if key.len() < 16 || read_u64(&key, 0)? != wanted { continue; }
        let kind = read_u32(&key, 8)?;
        let bucket = read_u32(&key, 12)?;
        let value = bytes(row.get("value"))?;
        if value.len() < 24 { continue; }
        out.push(MemoryHistogramBucket {
            kind: match kind { 1 => "page-fault", 2 => "direct-reclaim", _ => "unknown" }.into(),
            bucket,
...
fn read_cgroup_memory(path: &Path) -> Result<CgroupMemoryStats> {
    if !path.exists() { bail!("cgroup {} does not exist", path.display()); }
...
fn normalize_cgroup(path: &Path) -> PathBuf {
    if path.is_absolute() && path.starts_with("/sys/fs/cgroup") { path.to_path_buf() }
    else if path.is_absolute() { Path::new("/sys/fs/cgroup").join(path.strip_prefix("/").unwrap_or(path)) }
    else { Path::new("/sys/fs/cgroup").join(path) }
}

fn kv_u64(path: &Path) -> BTreeMap<String, u64> {
    fs::read_to_string(path).ok().map(|text| text.lines().filter_map(|line| {
        ...
        for line in text.lines() {
            ...
            for field in parts {
                if let Some((name, value)) = field.split_once('=') {
                    if name == "avg10" {
                        if let Ok(v) = value.parse() { out.insert(format!("{kind}.avg10"), v); }
                    }
                }
            }
        }
...
fn process_start_monotonic_ns(pid: u32) -> Result<(u64, u64)> {
    ...
    let start_ticks: u64 = fields.get(19).context("/proc pid/stat missing starttime")?.parse()?;
    let hz = Command::new("getconf").arg("CLK_TCK").output().ok()
...
unsafe extern "C" { fn clock_gettime(clockid: i32, tp: *mut Timespec) -> i32; }
...
fn monotonic_ns() -> Result<u64> {
    ...
    if ts.tv_sec < 0 || ts.tv_nsec < 0 { bail!("CLOCK_MONOTONIC returned a negative timestamp"); }
    Ok((ts.tv_sec as u64).saturating_mul(1_000_000_000).saturating_add(ts.tv_nsec as u64))
}

fn validate_phase(phase: &str) -> Result<()> {
    const PHASES: &[&str] = &[
        "guest-ready", "pause-request", "paused", "resume-request", "resumed",
        "snapshot-begin", "snapshot-end", "restore-begin", "restore-end",
    ];
    if PHASES.contains(&phase) { Ok(()) } else { bail!("invalid phase {phase}; expected one of {}", PHASES.join(", ")) }
}

fn marker_path(id: Uuid, root: &Path) -> PathBuf { root.join(format!("{id}.json")) }
fn read_markers(id: Uuid, root: &Path) -> Result<MarkerState> {
    let path = marker_path(id, root);
    let state: MarkerState = serde_json::from_slice(&fs::read(&path).with_context(|| format!("reading {}", path.display()))?)?;
    if state.vm_id != id { bail!("marker file VM id mismatch"); }
    Ok(state)
}

fn mem_map(root: &Path, name: &str) -> PathBuf { root.join("memprof/maps").join(name) }
fn helper_path(env_name: &str, installed: &str, fallback: &str) -> PathBuf {
    env::var(env_name).map(PathBuf::from).unwrap_or_else(|_| {
        ...
        if installed.exists() { installed } else { PathBuf::from(fallback) }
    })
}
fn command_exists(name: &str) -> bool {
    Command::new("sh").arg("-c").arg(format!("command -v -- {name} >/dev/null 2>&1")).status().map(|s| s.success()).unwrap_or(false)
}

fn bpftool_rows(path: &Path) -> Result<Vec<Value>> {
    let out = Command::new("bpftool").args(["-j", "map", "dump", "pinned"]).arg(path).output()
        .with_context(|| format!("dumping {}", path.display()))?;
    if !out.status.success() { bail!("bpftool dump {}: {}", path.display(), String::from_utf8_lossy(&out.stderr).trim()); }
    let value: Value = serde_json::from_slice(&out.stdout)?;
    value.as_array().cloned().ok_or_else(|| anyhow!("bpftool returned non-array JSON for {}", path.display()))
}

fn bytes(value: Option<&Value>) -> Result<Vec<u8>> {
    let arr = value.and_then(Value::as_array).context("bpftool key/value is not a byte array")?;
    arr.iter().map(|v| {
        if let Some(n) = v.as_u64() { return u8::try_from(n).context("bpftool byte >255"); }
...
fn nonzero(v: u64) -> Option<u64> { (v != 0).then_some(v) }
fn bucket_le_ns(bucket: u32) -> Option<u64> {
    if bucket as usize + 1 >= HIST_BUCKETS { None } else { Some(1000u64.saturating_mul(1u64 << bucket)) }
}

pub fn default_pin_root() -> PathBuf {
    env::var("FLUXVM_INTEL_PIN_ROOT").map(PathBuf::from).unwrap_or_else(|_| DEFAULT_PIN_ROOT.into())
}
pub fn default_marker_root() -> PathBuf {
    env::var("FLUXVM_MEMPROF_MARKER_ROOT").map(PathBuf::from).unwrap_or_else(|_| DEFAULT_MARKER_ROOT.into())
}
...
mod tests {
    ...
    fn boot_durations_are_monotonic_and_never_negative() {
        ...
        let boot = boot_profile(1_000_000_000, 100, KernelBootFirst { first_kvm_entry_ns: Some(1_500_000_000), ..Default::default() }, markers);
