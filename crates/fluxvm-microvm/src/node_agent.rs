...
use crate::images::{guest_image_host_path, guest_image_kernel_path, looks_like_direct_image};
...
pub async fn run(client: Client, fluxvm: FluxVMClient, node_name: String) {
    ...
    Controller::new(api, watcher::Config::default())
        .run(
            reconcile,
            |_o, e, _c| {
                tracing::warn!(error = %e, "node agent failed");
                Action::requeue(ERROR)
            },
            ctx,
        )
        .for_each(|res| async move {
            if let Err(e) = res {
                tracing::warn!(error = %e, "node agent error");
            }
        })
        .await;
}
...
async fn reconcile(obj: Arc<MicroVM>, ctx: Arc<Context>) -> Result<Action, Error> {
    ...
    let mine = obj.status.as_ref().and_then(|s| s.runtime.node.as_deref())
...
async fn apply(obj: &MicroVM, api: &Api<MicroVM>, ctx: &Context) -> Result<Action, Error> {
    ...
    let record = if let Some(id) = status.runtime.uuid.clone() {
        ...
        ctx.fluxvm
            .claim_pool(&pool_name, &name, obj.spec.ttl_seconds)
            .await?
    } else {
        status.phase = "Provisioning".into();
        let mut spec = obj.spec.clone();
        let (image, kernel) =
            resolve_image(&ctx.client, obj.namespace().as_deref(), &spec.image).await?;
        spec.image = image;
        ctx.fluxvm
            .create_vm(&name, &spec, kernel.as_deref())
            .await?
    };
    ...
    if obj.spec.command.is_some() && status.phase == "Running" && !status.command_ran {
        ...
        match ctx
            .fluxvm
            .exec(&id, cmd, obj.spec.command_timeout_seconds)
            .await
        {
            Ok(out) => {
                status.command_ran = true;
                status.command_exit = out
                    .get("exit_code")
                    .and_then(|v| v.as_i64())
                    .map(|i| i as i32);
                if !obj.spec.persist {
                    status.phase = if status.command_exit.unwrap_or(0) == 0 {
                        "Succeeded".into()
                    } else {
                        "Failed".into()
                    };
    ...
    if status.phase == "Running" && prev_phase != "Running" {
            ...
            .and_then(|a| a.get("microvm.fluxvm.zyvor.io/scheduled-at"))
        {
            ...
            if let Ok(d) = chrono::Utc::now().signed_duration_since(created.0).to_std() {
...
/// Resolve `spec.image` to `(host disk path, kernel path)`. A direct image
/// (path/URL/known extension) has no companion kernel -- that's only ever
/// carried by a GuestImage catalog entry, resolved here alongside the disk
/// image so a single lookup covers both.
async fn resolve_image(
    ...
    let img = api
        .get(image)
        .await
        .map_err(|e| Error::Flux(format!("GuestImage {namespace}/{image}: {e}")))?;
    let path = guest_image_host_path(&img).ok_or_else(|| {
        Error::Flux(format!(
            "GuestImage {namespace}/{image} is not Ready on this node (stage with GuestKit)"
        ))
    })?;
    Ok((path, guest_image_kernel_path(&img)))
}
