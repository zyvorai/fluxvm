...
// RuleHit is Set 17's exact rich-rule telemetry identity. RuleIndex is the
// current fluxvm_prules slot; Matched=false denotes the default-deny/audit
// miss sentinel rather than a real rule. Policy updates reset fluxvm_prhit,
// so a reused slot never carries another rule's historical counter.
type RuleHit struct {
	RuleIndex uint32
	Matched   bool
	Direction string
...
func (c Collector) collectVM(parent context.Context, vm string) VMState {
	...
	if st.PodID != 0 {
		...
		shared := c.podStats(ctx, filepath.Join(pin, "maps", "fluxvm_ppstat"), st.PodID, &st)
		st.EgressStats, st.IngressStats = shared, shared
		...
		if egress, ingress, hits, ok := c.ruleTelemetry(
			ctx,
			filepath.Join(pin, "maps", "fluxvm_prhit"),
			filepath.Join(pin, "maps", "fluxvm_prules"),
			st.PodID,
			&st,
		); ok {
			st.EgressStats, st.IngressStats = egress, ingress
			st.RuleHits = hits
			st.DirectionalStats = true
		}
	}
...
// this VM still runs an older schema-v8 object, in which case callers retain
// Set 15's shared fluxvm_ppstat fallback. A present but unreadable map is
...
func nativeU16(b []byte) uint16 {
	...
	return binary.NativeEndian.Uint16(b[:2])
}
