...
Schema v8 completes Set 13's Pod-scoped policy path with four additions, all still additive on top of schema v7's ABI:

- `fluxvm_pid4_cidr`/`fluxvm_pid6_cidr` (`BPF_MAP_TYPE_LPM_TRIE`, keyed like `fluxvm_tc.bpf.c`'s own VM-level `fluxvm_v4`/`fluxvm_v6` but with `pod_id` in place of `identity`): a peer with no hit in the address-wide `fluxvm_pid4`/`fluxvm_pid6` maps now falls through to a CIDR-prefix lookup before the exact-port maps, letting the NetworkPolicy controller compile a broad `ipBlock` into a real prefix allow instead of enumerating only currently-known cluster addresses.
- `fluxvm_pid4_port_range`/`fluxvm_pid6_port_range` (`BPF_MAP_TYPE_HASH`, keyed by `(pod_id, address, protocol)`, valued as a fixed 8-entry `(start, end)` array): a peer+protocol with no exact `fluxvm_pid4_port`/`fluxvm_pid6_port` hit falls through to a bounded inclusive-range scan before the pod-level `default_deny` fallback, letting the controller compile a NetworkPolicy `endPort` range instead of denying it.
- A fully parallel `_in`-suffixed map set (`fluxvm_pspol_in`, `fluxvm_pid4_in`/`fluxvm_pid6_in`, `fluxvm_pid4_port_in`/`fluxvm_pid6_port_in`, `fluxvm_pid4_cidr_in`/`fluxvm_pid6_cidr_in`, `fluxvm_pid4_port_range_in`/`fluxvm_pid6_port_range_in`, `fluxvm_ppstat_in`) for Pod-**ingress**-direction policy -- kept as independent map instances from the (pre-existing, egress-direction) maps above, same reasoning as Pod policy never sharing map instances with Service Fabric's VIP policy: independent lifecycle, independent sizing, zero risk of one direction's bug corrupting the other.
- A new `fluxvm_pod_ingress` program (`bpf/fluxvm_tc.bpf.c`), attached at the tc **egress** hook (host-side egress on a Pod VM's tap = traffic entering the VM) alongside the existing `fluxvm_egress` program (tc **ingress** hook, unchanged, still VM-egress-direction only). Reads the `_in` maps by `saddr` (peer) and destination port, the mirror of how `fluxvm_egress`'s pod-policy check reads the egress maps by `daddr`.

No existing map's ABI changed, and `fluxvm_egress`'s own logic (VM-level CIDR/L4/rate/conntrack enforcement, and its egress-direction pod-policy check) is untouched. Existing schema-v7 pins are considered incompatible and repaired by the existing attach/reconcile path, same as every prior schema bump. A `fluxvm_tc.bpf.o` built before this bump has none of the four additions above; `configure_pod_maps`/`configure_pod_ingress_maps` detect their absence and skip writing rather than failing the whole policy apply, the same graceful-degradation shape used for `fluxvm_pid4_port`/`fluxvm_pid6_port` predating Set 13's original port-rule support. `crates/fluxvm-network/src/ebpf.rs::apply()` similarly skips the `fluxvm_pod_ingress` attach entirely (both the TCX and legacy clsact/tc paths) when the program's pin is absent from a `loadall` load -- a VM attached against an older object simply never gets the ingress-direction hook, its (unchanged) egress enforcement working exactly as before.

`bpftool prog load` (singular) pins only the first `SEC("tc")` program it finds in an object's ELF order; with two `SEC("tc")` programs now in one object, `apply()` uses `bpftool prog loadall` instead, pinning every program in the object by its own function name -- `scripts/test-ebpf-smoke.sh` was updated the same way, since it loads the same object directly for its own kernel verifier proof.
