#!/usr/bin/env bash
# Render FluxVM social cards with Google Chrome + macOS `sips` (nothing to install).
...
SOCIAL_OUT="${1:-$HERE/fluxvm-social-card.jpg}"
SHARE_OUT="${2:-$HERE/fluxvm-share-card.png}"

PNG="$(mktemp "${TMPDIR:-/tmp}/fluxvm-social.XXXXXX.png")"
PNG2="$(mktemp "${TMPDIR:-/tmp}/fluxvm-share.XXXXXX.png")"
trap 'rm -f "$PNG" "$PNG2"' EXIT

"$CHROME" --headless=new --disable-gpu --hide-scrollbars --force-device-scale-factor=1 \
  --window-size=1600,900 --screenshot="$PNG" "file://$HERE/fluxvm-social-card.html" >/dev/null 2>&1
...
"$CHROME" --headless=new --disable-gpu --hide-scrollbars --force-device-scale-factor=1 \
  --window-size=1200,630 --screenshot="$PNG2" "file://$HERE/fluxvm-share-card.html" >/dev/null 2>&1
