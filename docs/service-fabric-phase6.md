...
|-----|------------|--------|
| `SLO_VIP_P99_MS` | `25` when `VIP=` set (CI); `50` (`SLO_LAB=1`); else skip | VIP connect p99 ≤ threshold |
...
| `SLO_CPU_MAX_PERCENT` | unset (CI) / `85` (`SLO_LAB=1`) | fluxvm process CPU% avg during storm ≤ ceiling; soft-skip if no pid |
