...
| **DP** | Direct (bridge-less) datapath: Cilium-style redirect between the Pod veth / host uplink and the guest tap | **Done, opt-in** — shim `FLUXVM_CONTAINER_CNI_DATAPATH=bridge\|direct\|auto` (default `bridge`), daemon loader in the Pod netns (TCX + legacy tc), `l2-uplink` standalone mode (shared per-uplink maps, ARP steering, MicroVM CRD `networkMode: direct`), warm-pool hotplug via QMP fd-passing, Linux 7.x verifier fix, hop view, capability probe, bench + evidence scripts. **Live Cilium + KVM evidence pending** — the default flips to `auto` only after `FLUXVM_DIRECT_LIVE=1` passes; Multus secondaries stay on the bridge chain | [direct-datapath.md](direct-datapath.md) |
