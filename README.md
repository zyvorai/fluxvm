...
- **Service Fabric** — node-local Maglev VIP load balancing (**schema 4 ABI / program generation 6 / v6 policy+HA queue**): dual-stack NAT/DSR/SNAT, forward conntrack affinity, Ready/Draining/Unhealthy + TCP health, VIP advertisement snapshot, incremental intent reconcile, bounded HA delta journal + mutation queue, per-service EDT, FluxScope flows, optional host-routing, identity/L7 service policy. Fabric owns leases/fan-out/replication/policy transactions. See [docs/service-fabric.md](docs/service-fabric.md).
